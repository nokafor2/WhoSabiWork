<?php
/* 
Seller Class for Ayuanorama database containing functions to get data from the database.
*/
...
class Seller {
	...
	protected static $db_fields = array('id', 'customers_id', 'provisions', 'fabrics', 'electronics', 'electricals_and_accessories', 'computers_and_accessories', 'phones_and_accessories', 'kitchen_utensils', 'house_hold_equipment', 'jeweries', 'shoes', 'clothes_and_apparels', 'televisions', 'bags_and_boxes', 'tiles', 'building_materials', 'furniture', 'cars', 'motorcycles', 'tricycles', 'paints', 'books', 'decors', 'generators', 'watches', 'hairs_and_wigs', 'exercise_equipment', 'plumbing_materials', 'fashion_accessories', 'pharmaceuticals', 'fragrances_and_ornamentals', 'fishrey', 'skin_care_products', 'hair_products', 'baby_care_accessories');
	// Every column in the seller table have an attribute
	// These values gets instantiated when the any of the SQL request is asked.
	public $id, $customers_id;
	public $provisions, $fabrics, $electronics, $electricals_and_accessories, $computers_and_accessories, $phones_and_accessories, $kitchen_utensils, $house_hold_equipment, $jeweries, $shoes, $clothes_and_apparels, $televisions, $bags_and_boxes, $tiles, $building_materials, $furniture, $cars, $motorcycles, $tricycles, $paints, $books, $decors, $generators, $watches, $hairs_and_wigs, $exercise_equipment, $plumbing_materials, $fashion_accessories, $pharmaceuticals, $fragrances_and_ornamentals, $fishrey, $skin_care_products, $hair_products, $baby_care_accessories;
	
	// This function returns all the sellers from the sellers table in a database. It will also return them as an array of objects.
	public static function find_all() {
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM sellers"
		return static::find_by_sql("SELECT * FROM ".static::$table_name);
    }
  
    // This function will return a single seller record from the seller table in a database. It will also return them as objects.
    public static function find_by_id($id=0) {
		global $database;
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM sellers WHERE id={$id} LIMIT 1"
	...
	public function selected_choices($customerID) {
		...
		$sellers = array_keys($this->getSellers());
		foreach ($rowData as $key => $value) {
			if (in_array($key, $sellers) && $value == TRUE) {
	...
	// Get the Sellers which are attributes of this class.
	public function getSellers() {
		...
		// Sort the sellers alphabetically
		ksort($slicedArray);

		// Return the array keys only which will be the sellers
		$sellers = array_keys($slicedArray);
	...
	// Create a seller record and insert it to the database
	...
	// update a seller record in the database
	...
	public function delete() {
		...
		// NB: After deleting, the instance of seller still 
		// exists, even though the database entry does not.
		// This can be useful, as in:
		//   echo $seller->first_name . " was deleted";
		// but, for example, we can't call $seller->update() 
		// after calling $seller->delete().
	}
	
	function sellersTypeOptions() {
		// get an array of the seller types
		// $sellers = array_keys($this->getSellers());
		
		// Get the available sellers registered
		$sellers = $this->getAvailableSellers(); 
		...
		foreach ($sellers as $seller) {
			$output .= "<option value='{$seller}'>".ucfirst(str_replace("_", " ", $seller))."</option>";
		}
	...
	function getAvailableSellers() {
		...
		// Generate SQL query to get available sellers
		// $sql = "SELECT ";
		// $sql .= join(", ", array_values($sellers));
		// $sql .= " FROM sellers";
		...
		$sql .= "SELECT customers_id FROM business_categories WHERE business_categories.seller = 1 AND business_categories.customers_id IN (";
		$sql .= "SELECT customers_id FROM ".static::$table_name.")))";
		echo $sql;
		$result_set = $database->query($sql);
		
		$selectedSellers = array();
		// get the various sellers in a 2x2 array or table
		while($row = mysqli_fetch_assoc($result_set)){ 
		...
		foreach ($sellers as $key => $value) {
			$selectedColumn = array_column($selectedSellers, $value);
			if (in_array(1, $selectedColumn)) {
				$availableSellers[] = $value;
			}
		}

		$ucfAvailableSellers = array();
		// capitalize the first character of each word
		foreach ($availableSellers as $key => $value) {
			$ucfAvailableSellers[] = ucfirst($value);
		}
		// Sort the arrays alphabetically in the array
		sort($ucfAvailableSellers);
		return $ucfAvailableSellers;
	}
}
