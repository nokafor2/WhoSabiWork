<?php
/* 
Technical services Class for Ayuanorama database containing functions to get data from the database.
*/
...
class Technical_Service {
	...
	protected static $table_name = "technical_services";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'customers_id', 'engine_service', 'mechanical_service', 'electrical_service', 'air_conditioning_service', 'computer_diagnostics_service', 'panel_beating_service', 'body_work_service', 'shock_absorber_service', 'ballon_shocks_service', 'wheel_balancing_and_alignment_service', 'car_wash_service', 'towing_service');
	// Every column in the technical_services table have an attribute
	...
	public $air_conditioning_service;
	public $computer_diagnostics_service;
	public $panel_beating_service;
	public $body_work_service;
	public $shock_absorber_service;
	public $ballon_shocks_service;
	public $wheel_balancing_and_alignment_service;
	public $car_wash_service;
	public $towing_service;
	
	// This function returns all the technical_services from the technical_services table in a database. It will also return them as an array of objects.
	public static function find_all() {
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM technical_services"
		return static::find_by_sql("SELECT * FROM ".static::$table_name);
    }
  
    // This function will return a single technical_services record from the technical_services table in a database. It will also return them as objects.
    public static function find_by_id($id=0) {
		global $database;
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM technical_services WHERE id={$id} LIMIT 1"
	...
	public function selected_choices($customerID) {
		...
		$serviceTypes = array_keys($this->getTechnicalServices());
	...
	// Get the vehicle types which are attributes of this class.
	public function getTechnicalServices() {
		...
		$technical_services = array_keys($slicedArray);
		// Sort the array alphabetically
		// asort($technical_services);
		
		// return alphabetically sorted technical services;
		return array_flip($technical_services); // Use the array keys so that it will be recovered from json.
	}
	...
	// Create a technical_services record and insert it to the database
	...
	// update a technical_service record in the database
	...
	public function delete() {
		...
		// NB: After deleting, the instance of Technical_Services still 
		// exists, even though the database entry does not.
		// This can be useful, as in:
		//   echo $technical_services->first_name . " was deleted";
		// but, for example, we can't call $technical_services->update() 
		// after calling $technical_services->delete().
	}

	public function getAvailableTechServ($vehicleType, $vehicleBrand, $businessCategory) {
		global $database;
		...
		// get the various tecnical services from the database
		$technicalServices = $this->getTechnicalServices();
		// get an array of the carBr types
		$technicalServices = array_keys($technicalServices);
		...
		// Generate SQL query to get available technical Services
		...
		// get the various technicalServices in a 2x2 array or table
		while($row = mysqli_fetch_assoc($result_set)){ 
			// Gets the rows of the table 
			$selectedTechServ[] = $row; 
		}

		$availableTechServ = array();
		foreach ($technicalServices as $key => $value) {
			$selectedColumn = array_column($selectedTechServ, $value);
			if (in_array(1, $selectedColumn)) {
				$availableTechServ[] = $value;
			}
		}

		$ucfAvailableTechServ = array();
		// capitalize the first character of each word
		foreach ($availableTechServ as $key => $value) {
			$ucfAvailableTechServ[] = ucfirst($value);
		}
		// Sort the arrays alphabetically in the array
		sort($ucfAvailableTechServ);
		return $ucfAvailableTechServ;
	}
}
