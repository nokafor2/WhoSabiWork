<?php
/* 
Photograph_reply Class for WhoSabiWork.com database containing functions to get data from the database.
*/

require_once(LIB_PATH.DS.'database.php');

class Photograph_Reply {
	
	// This is variable will be used to dynamically choose a table from the database
	protected static $table_name = "photographs_replies";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'comment_id', 'photograph_id', 'customers_id', 'public_user_id', 'public_customer_id', 'reply', 'date_created');
	// Every column in the photograph_reply table have an attribute
	// These values gets instantiated when any of the SQL request is asked.
	public $id;
	public $comment_id;
	public $photograph_id;
	public $customers_id;
	public $public_user_id;
	public $public_customer_id;
	public $reply;
	public $date_created;

	// "new" is a reserved word so we use "make" (or "build")
	public static function make($commentId, $photographId, $customersId, $userOrCusId, $reply="") {
		global $session;
		
		if(!empty($commentId) && !empty($photographId) && !empty($customersId) && !empty($userOrCusId) && !empty($reply)) {
			// an instance of a class is created in this fuction because the function is a static one. The this keyword can't be used in here. so the created instance will be used to reference the variables in the Photograph_Reply class.
			$photoReply = new Photograph_Reply();
			$photoReply->comment_id = $commentId;
			$photoReply->photograph_id = $photographId;
			$photoReply->customers_id = $customersId;
			if ($session->is_user_logged_in()) {
				$photoReply->public_user_id = $userOrCusId;
			} elseif($session->is_customer_logged_in()) {
				$photoReply->public_customer_id = $userOrCusId;
			}
			$photoReply->date_created = strftime("%Y-%m-%d %H:%M:%S", time());
			$photoReply->reply = $reply;
	...
	// This function returns all the photograph_replies from the photographs_replies table in a database. It will also return them as an array of objects.
	public static function find_all() {
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM photographs_replies"
		return static::find_by_sql("SELECT * FROM ".static::$table_name);
    }
  
    // This function will return a single photograph_reply record from the photographs_replies table in a database. It will also return them as objects.
    public static function find_by_id($id=0) {
		global $database;
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM photographs_replies WHERE id={$id} LIMIT 1"
    ...
    // Get the number of replies for the photograph
    public static function count_photograph_replies($commentId=0, $photographId=0, $customerId=0) {
    	global $database;
		$commentId = $database->escape_value($commentId);
		$photographId = $database->escape_value($photographId);
		$customerId = $database->escape_value($customerId);

    	$sql = "SELECT COUNT(*) FROM ".static::$table_name." WHERE comment_id={$commentId} AND photograph_id={$photographId} AND customers_id={$customerId}";
    ...
    public static function find_photograph_replies($commentId=0) {
		global $database;
		$commentId = $database->escape_value($commentId);
		// $photographId = $database->escape_value($photographId);
		// $customerId = $database->escape_value($customerId);
		
		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE comment_id={$commentId} ORDER BY date_created ASC");
		
		return !empty($result_array) ? $result_array : false;
    }

    // Find all the photograph replies using the photograph id
    public static function find_photograph_replies_by_photoId($photographId) {
    	global $database;
		$photographId = $database->escape_value($photographId);

		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE photograph_id={$photographId}");
		
		return $result_array;
    }

    public static function find_photograph_replied_by_user($commentId=0, $photographId=0, $public_user_id) {
		global $database;
		$commentId = $database->escape_value($commentId);
		$photographId = $database->escape_value($photographId);
		$public_user_id = $database->escape_value($public_user_id);
		
		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE comment_id={$commentId} AND photograph_id={$photographId} AND public_user_id={$public_user_id}  ORDER BY date_created DESC");
		
		return !empty($result_array) ? $result_array : false;
    }

	public static function find_photograph_replied_by_customer($commentId=0, $photographId=0, $public_customer_id) {
		global $database;
		$commentId = $database->escape_value($commentId);
		$photographId = $database->escape_value($photographId);
		$public_customer_id = $database->escape_value($public_customer_id);
		
		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE comment_id={$commentId} AND photograph_id={$photographId} AND public_customer_id={$public_customer_id}  ORDER BY date_created DESC");
	...
	private static function instantiate($record) {
		...
		$object = new $class_name; // This is used to make an instantiation of the Class. Its similar to ($photographs_replies = new photographs_replies();)
	...
	// Create a photograph_reply record and insert it to the database
	...
	// update a photograph_reply record in the database
	...
	public function delete() {
		...
		// NB: After deleting, the instance of photograph_reply still 
