<?php
/* 
Advocator Class for WhoSabiWork.com database containing functions to get data from the database.
*/
...
class Advocate {
	...
	protected static $table_name = "advocates";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'customers_id', 'user_advocator', 'customer_advocator', 'date_created');
	// Every column in the Advocator table have an attribute
	// These values gets instantiated when any of the SQL request is asked.
	public $id;
	public $customers_id;
	public $user_advocator;
	public $customer_advocator;
	public $date_created;
	
	// This function returns all the advocators from the advocators table in a database. It will also return them as an array of objects.
	public static function find_all() {
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM advocators"
		return static::find_by_sql("SELECT * FROM ".static::$table_name);
  }
  
  // This function will return a single advocator record from the advocator table in a database. It will also return them as objects.
  public static function find_by_id($id=0) {
		global $database;
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM advocators WHERE id={$id} LIMIT 1"
  ...
  // check for empty ids that has not been saved into
	...
	// Find the advocators of a customer by a user
  public static function find_user_advocator($customerId=0, $public_user_id) {
		global $database;
		$customerId = $database->escape_value($customerId);
		$public_user_id = $database->escape_value($public_user_id);
		
		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE customers_id={$customerId} AND user_advocator={$public_user_id} LIMIT 1");
		
		return !empty($result_array) ? array_shift($result_array) : false;
		// return $result_array;
  }

  // Count the number of advocators for a customer
  public static function count_advocators($customerId) {
    	global $database;
		$customerId = $database->escape_value($customerId);

		$sql = "SELECT COUNT(*) FROM ".static::$table_name." WHERE customers_id={$customerId}";
  ...
  // Find the advocators of a customer by a customer
	public static function find_customer_advocator($customerId=0, $public_customer_id) {
		global $database;
		$customerId = $database->escape_value($customerId);
		$public_customer_id = $database->escape_value($public_customer_id);
		
		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE customers_id={$customerId} AND customer_advocator={$public_customer_id} LIMIT 1");
		
		return !empty($result_array) ? array_shift($result_array) : false;
		// return $result_array;
  }

  // return an array of all the customers advocated by the user id
  public static function find_all_advocated_by_user($public_user_id) {
  	global $database;
		$public_user_id = $database->escape_value($public_user_id);

		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE user_advocator={$public_user_id}");
		
		return $result_array;
  }

  // return an array of all the customers advocated by the customer id 
  public static function find_all_advocated_by_customer($public_customer_id) {
    	global $database;
		$public_customer_id = $database->escape_value($public_customer_id);

		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE customer_advocator={$public_customer_id}");
		
		return $result_array;
  }

  // return an array of all the customers advocated by the user id with the set limit and offset
  public static function find_by_limit_advocated_by_user($public_user_id, $limit, $offset) {
    	global $database;
		$public_user_id = $database->escape_value($public_user_id);
		$limit = $database->escape_value($limit);
		$offset = $database->escape_value($offset);

		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE user_advocator={$public_user_id} LIMIT {$limit} OFFSET {$offset}");
		
		return $result_array;
  }

  // return an array of all the customers advocated by the customer id with the set limit and offset
  public static function find_by_limit_advocated_by_customer($public_customer_id, $limit, $offset) {
    	global $database;
		$public_customer_id = $database->escape_value($public_customer_id);
		$limit = $database->escape_value($limit);
		$offset = $database->escape_value($offset);

		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE customer_advocator={$public_customer_id} LIMIT {$limit} OFFSET {$offset}");
	...
	public static function find_by_column_name($columnName, $variable) {
		...
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM advocatorss WHERE id={$id} LIMIT 1"
		// 'self' is replaced with 'static' because of use of late static binding to occur at run time.
  ...
  public static function find_by_sql($sql="") {
	...
	private static function instantiate($record) {
		...
		$object = new $class_name; // This is used to make an instantiation of the Class. Its similar to ($advocators = new Advocator();)
	...
	// Create an advocator record and insert it to the database
	...
	// update an advocator record in the database
	...
	public function delete() {
		...
		// NB: After deleting, the instance of Advocator still 
