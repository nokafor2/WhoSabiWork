...
class Admin_Failed_Login {
	
	// This is variable will be used to dynamically choose a table from the database
	protected static $table_name = "admin_failed_logins";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'admin_id', 'username', 'email', 'phone_number', 'record', 'last_time');
  ...
  public static function find_by_sql($sql="") {
	...
	public function record_admin_failed_login($loginId) {
		global $database;
		$loginId = $database->escape_value($loginId);
		$loginIdType = getLoginIdType($loginId);

		if ($loginIdType === 'username') {
			$adminStatic = Admin::find_by_column_name('username', $loginId);
			// Get an array of username, email and phone_number
			if (!empty($adminStatic)) {
				$failed_login = $this->getFailedLoginRecord($adminStatic, $loginId, $loginIdType); 

				if (empty($failed_login)) {
					// Create a record of a new failed login
					$this->admin_id = $adminStatic->id;
					$this->username = $adminStatic->username;
					$this->email = NuLL;
					...
					if ($failed_login->record > $this->throttle_at) {				
						...
						$failed_login->username = $adminStatic->username;
				...
				$failed_login = Admin_Failed_Login::find_by_column_name('username', $loginId);
				if (empty($failed_login)) {
					// Create a new record for a username that doesn't exist in the Admin table
					$this->admin_id = NuLL;
			...
			$adminStatic = Admin::find_by_column_name('admin_email', $loginId);
			// Get an array of username, email and phone_number
			if (!empty($adminStatic)) {
				$failed_login = $this->getFailedLoginRecord($adminStatic, $loginId, $loginIdType);
				...
				if (empty($failed_login)) {
					...
					$this->admin_id = $adminStatic->id;
					$this->username = NuLL;
					$this->email = $adminStatic->admin_email;
					$this->phone_number = NuLL;
					$this->record = 1; 
					$this->last_time = $this->current_Date_Time();
					$this->create();
					...
					if ($failed_login->record > $this->throttle_at) {				
						...
						$failed_login->email = $adminStatic->admin_email;
						$failed_login->phone_number = NuLL;
						$failed_login->record = 1; 
						$failed_login->last_time = $failed_login->current_Date_Time();
						$failed_login->update();
				...
				$failed_login = Admin_Failed_Login::find_by_column_name('email', $loginId);
				if (empty($failed_login)) {
					// Create a new record for a username that doesn't exist in the Admin table
					$this->admin_id = NuLL;
					$this->username = NuLL;
			...
			$adminStatic = Admin::find_by_column_name('phone_number', $loginId);
			// Get an array of username, email and phone_number
			if (!empty($adminStatic)) {
				$failed_login = $this->getFailedLoginRecord($adminStatic, $loginId, $loginIdType);
				...
				if (empty($failed_login)) {
					...
					$this->username = NuLL;
					$this->email = NuLL;
					$this->phone_number = $adminStatic->phone_number;
					$this->record = 1; 
					$this->last_time = $this->current_Date_Time();
					$this->create();
				} else {
					...
					if ($failed_login->record > $this->throttle_at) {				
						...
						$failed_login->username = NuLL;
						$failed_login->email = NuLL;
						$failed_login->phone_number = $adminStatic->phone_number;
						$failed_login->record = 1; 
						$failed_login->last_time = $failed_login->current_Date_Time();
						$failed_login->update();
					} else {
				...
				$failed_login = Admin_Failed_Login::find_by_column_name('phone_number', $loginId);
				if (empty($failed_login)) {
					// Create a new record for a username that doesn't exist in the Admin table
					$this->admin_id = NuLL;
					$this->username = NuLL;
	...
	public function clear_admin_failed_logins($loginId) {
		global $database;
		$loginId = $database->escape_value($loginId);
		$loginIdType = getLoginIdType($loginId);
	...
	public function throttle_failed_admin_logins($loginId) {
		$delay_in_minutes = 10;
	...
	public function getFailedLoginRecord($adminStatic, $loginId, $loginIdType) {
		// Get an array of username, email and phone_number
		if ($loginIdType === 'username') {
			$adminLoginDetails = array();
			$adminLoginDetails['username'] = $loginId;
			$adminLoginDetails['email'] = $adminStatic->admin_email;
			$adminLoginDetails['phone_number'] = $adminStatic->phone_number;
		} elseif ($loginIdType === 'email') {
			$adminLoginDetails = array();
			$adminLoginDetails['username'] = $adminStatic->username;
			$adminLoginDetails['email'] = $loginId;
			$adminLoginDetails['phone_number'] = $adminStatic->phone_number;
		} else {
			$adminLoginDetails = array();
			$adminLoginDetails['username'] = $adminStatic->username;
			$adminLoginDetails['email'] = $adminStatic->admin_email;
			$adminLoginDetails['phone_number'] = $loginId;
		}			

		$failed_login = array();
		foreach ($adminLoginDetails as $loginIdPart => $value) {
