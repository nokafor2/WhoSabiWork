...
class User_Comment {
	...
	protected static $table_name="users_comments";
	protected static $db_fields=array('id', 'user_id_comment', 'customer_id_comment', 'customers_id', 'author', 'body', 'created');

	public $id;
	public $user_id_comment;
	public $customer_id_comment;
	public $customers_id;
	public $author;
	public $body;
	public $created;
	...
	public static function make($customersId, $userOrCusId, $author="Anonymous", $body="") {
		global $session;
		
		if(!empty($customersId) && !empty($userOrCusId) && !empty($author) && !empty($body)) {
			// an instance of a class is created in this fuction because the function is a static one. The this keyword can't be used in here. so the created instance will be used to reference the variables in the User_Comment class.
			$comment = new User_Comment();
			$comment->customers_id = $customersId;
			if ($session->is_user_logged_in()) {
				$comment->user_id_comment = $userOrCusId;
				// $comment->customer_id_comment = NULL;
			} elseif($session->is_customer_logged_in()) {
				$comment->customer_id_comment = $userOrCusId;
				// $comment->user_id_comment = NULL;
			}
	...
	// The comments found will be ordered by decending order from the most recent comments to the oldest
	public static function find_comments_on($customersId=0) {
		global $database;
	...
	// Find all the comments made by a user on a customer
	public static function find_comments_made_by_user($userOrCusId=0, $accountType="") {
		global $database;
		$userOrCusId = $database->escape_value($userOrCusId);
		$accountType = $database->escape_value($accountType);
		if ($accountType === "user") {
			$userOrCusIdComment = "user_id_comment";
		} else {
			$userOrCusIdComment = "customer_id_comment";
		}
		
		$sql = "SELECT * FROM " . self::$table_name;
		$sql .= " WHERE ".$userOrCusIdComment."=" .$userOrCusId;
		$sql .= " ORDER BY created DESC";
		// echo $sql;
		return self::find_by_sql($sql);
	}
	
	// Find all the comments made by a user on a particular customer
	public static function find_comments_made_by_user_to_customer() {
		return true;
	}
	...
	// Count all the comments from a particular customer using the customer's id
	// This will only return the number of pictures counted
