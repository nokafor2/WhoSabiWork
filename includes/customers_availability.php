<?php
/* 
Customers_Availability Class for Ayuanorama database containing functions to get data from the database.
*/
...
class Customers_Availability {
	
	// This is variable will be used to dynamically choose a table from the database
	protected static $table_name = "customers_availability";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'customers_id', 'date_available', 'eight_to_nine_am', 'nine_to_ten_am', 'ten_to_eleven_am', 'eleven_to_twelve_pm', 'twelve_to_one_pm', 'one_to_two_pm', 'two_to_three_pm', 'three_to_four_pm', 'four_to_five_pm', 'five_to_six_pm', 'six_to_seven_pm', 'seven_to_eight_pm', 'eight_to_nine_pm', 'nine_to_ten_pm', 'date_created');
	// Every column in the Customers_Availability table have an attribute
	// These values gets instantiated when the any of the SQL request is asked.
	public $id;
	public $customers_id;
	public $date_available;
	public $eight_to_nine_am;
	public $nine_to_ten_am;
	public $ten_to_eleven_am;
	public $eleven_to_twelve_pm;
	public $twelve_to_one_pm;
	public $one_to_two_pm;
	public $two_to_three_pm;
	public $three_to_four_pm;
	public $four_to_five_pm;
	public $five_to_six_pm;
	public $six_to_seven_pm;
	public $seven_to_eight_pm;
	public $eight_to_nine_pm;
	public $nine_to_ten_pm;
	public $date_created;
  ...
  // This function will return a single Customers_Availability record from the Customers_Availability table in a database. It will also return them as objects.
  public static function find_by_id($id=0) {
	...
	// Get the input of the customer from the customers_availability table using the customers_id
	...
	public static function find_by_date_available($customers_id=0, $date_available='') {
		...
		// query the database for a record using the customers_id
		$result_array = self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE customers_id=".$customers_id." AND date_available='".$date_available."'");
		return !empty($result_array) ? array_shift($result_array) : false;
	}
	
	// This function will return a single user record from the Customers_Availability table in a database using the username. It will also return them as objects.
	...
	private static function instantiate($record) {
		...
		$object = new $class_name; // This is used to make an instantiation of the Class. Its similar to ($Customers_Availability = new Customers_Availability();)
	...
	public function convertDbVarToFormTime($timesFromDb) {
		...
		foreach ($timesFromDb as $dbTime) {
			...
			} elseif ($dbTime === 'four_to_five_pm') {
				$timesForForm[$count] = '4:00 PM - 5:00 PM';
			} elseif ($dbTime === 'five_to_six_pm') {
				$timesForForm[$count] = '5:00 PM - 6:00 PM';
			} elseif ($dbTime === 'six_to_seven_pm') {
				$timesForForm[$count] = '6:00 PM - 7:00 PM';
			} elseif ($dbTime === 'seven_to_eight_pm') {
				$timesForForm[$count] = '7:00 PM - 8:00 PM';
			} elseif ($dbTime === 'eight_to_nine_pm') {
				$timesForForm[$count] = '8:00 PM - 9:00 PM';
			} elseif ($dbTime === 'nine_to_ten_pm') {
				$timesForForm[$count] = '9:00 PM - 10:00 PM';
			}
			$count++;
		}
