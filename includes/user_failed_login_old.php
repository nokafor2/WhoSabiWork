...
class User_Failed_Login {
	...
	protected static $db_fields = array('id', 'user_id', 'user_username', 'email', 'phone_number', 'record', 'last_time');
  ...
  public static function find_by_sql($sql="") {
	...
	/* public function record_user_failed_login($loginId) {
		global $database;
		$loginId = $database->escape_value($loginId);
		$loginIdType = getLoginIdType($loginId);
		if ($loginIdType === 'username') {
			$failed_login = static::find_by_column_name('user_username', $loginId);
		...
		// Check if failed attempt exists
		if(empty($failed_login)) {
			...
			if (!empty($userStatic)) {
				...
				$userLogin_id = $userStatic->id;
				if ($loginIdType === 'username') {
					$loginUsername = $userStatic->username;
					$loginEmail = NuLL;
					$loginPhoneNumber = NuLL;
				} elseif ($loginIdType === 'email') {
					$loginUsername = NuLL;
					$loginEmail = $userStatic->user_email;
					$loginPhoneNumber = NuLL;
				} else {
					$loginUsername = NuLL;
					$loginEmail = NuLL;
					$loginPhoneNumber = $userStatic->phone_number;
				}					
			} else {
			...
			$this->user_id = $userLogin_id;
			$this->user_username = $loginUsername;
			$this->email = $loginEmail; 
			$this->phone_number = $loginPhoneNumber; 
			$this->record = 1; 
			$this->last_time = $this->current_Date_Time();
			$this->create();
		} else {
			// Failed login already exist
	...
	public function record_user_failed_login($loginId) {
		...
		if ($loginIdType === 'username') {
			...
			if (!empty($userStatic)) {
				...
				$_SESSION['failed_login_record'] = $failed_login;
				// Determine the key which is save too
				$cell = 0;
				...
				if (empty($failed_login)) {
					...
					// Check if it is greater than the allowed failed attempt
					if ($failed_login[$cell]->record >= $this->throttle_at) {				
						// Restart an existing failed_login record
						$failed_login[$cell]->user_username = $userStatic->username;
						$failed_login[$cell]->email = NuLL;
						$failed_login[$cell]->phone_number = NuLL;
						$failed_login[$cell]->record = 1; 
						$failed_login[$cell]->last_time = $failed_login[$cell]->current_Date_Time();
						$failed_login[$cell]->update();
					} else {
						// Update an existing failed_login record
						$failed_login[$cell]->user_username = $userStatic->username;
						$failed_login[$cell]->email = NuLL;
						$failed_login[$cell]->phone_number = NuLL;
						$failed_login[$cell]->record = $failed_login[$cell]->record + 1; 
						$failed_login[$cell]->last_time = $failed_login[$cell]->current_Date_Time();
						$failed_login[$cell]->update();
					}
				}
				...
				if (empty($failed_login)) {
					...
					if ($failed_login->record >= $this->throttle_at) {				
						...
						// Update an existing failed_login record
						$failed_login->user_username = $loginId;
						$failed_login->email = NuLL;
						$failed_login->phone_number = NuLL;
						$failed_login->record = $failed_login->record + 1; 
						$failed_login->last_time = $failed_login->current_Date_Time();
			...
			if (!empty($userStatic)) {
				$userLoginDetails = array();
				$userLoginDetails['username'] = $userStatic->username;
				$userLoginDetails['email'] = $loginId;
				$userLoginDetails['phone_number'] = $userStatic->phone_number;
				...
				if (empty($failed_login)) {
					...
					$this->email = $userStatic->user_email;
					$this->phone_number = NuLL;
					$this->record = 1; 
					$this->last_time = $this->current_Date_Time();
					$this->create();
					...
					if ($failed_login[$cell]->record >= $this->throttle_at) {				
						// Restart an existing failed_login record
						$failed_login[$cell]->user_username = NuLL;
						$failed_login[$cell]->email = $userStatic->user_email;
						$failed_login[$cell]->phone_number = NuLL;
						$failed_login[$cell]->record = 1; 
						$failed_login[$cell]->last_time = $failed_login[$cell]->current_Date_Time();
						$failed_login[$cell]->update();
				...
				// Email doen't exist in the user's table, create a new record
				// Create a record of a new failed login
				// Check if the new email exists in the database
				$failed_login = User_Failed_Login::find_by_column_name('email', $loginId);
				if (empty($failed_login)) {
					// Create a new record for a username that doesn't exist in the User table
					$this->user_id = NuLL;
					$this->user_username = NuLL;
					...
					if ($failed_login->record >= $this->throttle_at) {				
						// Restart an existing failed_login record
						$failed_login->user_username = NuLL;
						$failed_login->email = $userStatic->email;
						$failed_login->phone_number = NuLL;
						$failed_login->record = 1; 
						$failed_login->last_time = $failed_login->current_Date_Time();
						$failed_login->update();
			...
			if (!empty($userStatic)) {
				...
				$userLoginDetails['username'] = $userStatic->username;
				$userLoginDetails['email'] = $userStatic->user_email;
				$userLoginDetails['phone_number'] = $loginId;
				...
				if (empty($failed_login)) {
					...
					$this->user_username = NuLL;
					$this->email = NuLL;
					$this->phone_number = $userStatic->phone_number;
					$this->record = 1; 
					$this->last_time = $this->current_Date_Time();
					$this->create();
				} else {
					...
					if ($failed_login[$cell]->record >= $this->throttle_at) {				
						...
						$failed_login[$cell]->user_username = NuLL;
						$failed_login[$cell]->email = NuLL;
						$failed_login[$cell]->phone_number = $userStatic->phone_number;
						$failed_login[$cell]->record = 1; 
						$failed_login[$cell]->last_time = $failed_login[$cell]->current_Date_Time();
						$failed_login[$cell]->update();
					} else {
				...
				// Phone number doen't exist in the user's table, create a new record
				// Create a record of a new failed login
	...
	public function clear_user_failed_logins($loginId) {
		global $database;
		$loginId = $database->escape_value($loginId);
		$loginIdType = getLoginIdType($loginId);
		if ($loginIdType === 'username') {
	...
	public function throttle_failed_user_logins($loginId) {
		$delay_in_minutes = 1;
