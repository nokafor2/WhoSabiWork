<?php
/* 
Business Category Class for Ayuanorama database contains functions to get data from the database.
*/
...
class Business_Category {
	...
	protected static $table_name = "business_categories";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'customers_id', 'technician', 'spare_part_seller', 'artisan', 'seller', 'business_description', 'views', 'advertise');
	// Every column in the business category table have an attribute
	// These values gets instantiated when the any of the SQL request is asked.
	public $id;
	public $customers_id;
	public $technician;
	public $spare_part_seller;
	public $artisan;
	public $seller;
	...
	// This function returns all the business categories from the business_categories table in a database. It will also return them as an array of objects.
	public static function find_all() {
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM business_categories"
		return static::find_by_sql("SELECT * FROM ".static::$table_name);
  }
  
  // This function will return a single business category record from the business category table in a database. It will also return them as objects.
  public static function find_by_id($id=0) {
		global $database;
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM business_categories WHERE id={$id} LIMIT 1"
  ...
  public static function find_by_sql($sql="") {
	...
	// Get the input of the customer from the business_categories table using the customers_id
	...
	// This function will get all the customers who have paid for advertisement within a limit after an offset
	public static function find_cus_advertisements($limit, $offset) {
		global $database;
		$limit = $database->escape_value($limit);
		$offset = $database->escape_value($offset);

		$sql = "SELECT * FROM ".static::$table_name." WHERE advertise = 1 AND customers_id IN (";
		...
		// Return the paid customer advertisers
	...
	public function selected_business_categoty(){
		if (!is_null($this->technician) AND $this->technician == TRUE) {
			return "Technician";
		} elseif (!is_null($this->spare_part_seller) AND $this->spare_part_seller == TRUE) {
			return "Spare part seller";
		} elseif (!is_null($this->artisan) AND $this->artisan == TRUE) { 
			return "Artisan";
		} elseif (!is_null($this->seller) AND $this->seller == TRUE) { 
			return "Seller";
		} else {
			return FALSE;
		}
		
		/* if (isset($this->technician) AND $this->technician == TRUE) {
			return "Technician";
		} elseif (isset($this->spare_part_seller) AND $this->spare_part_seller == TRUE) {
			return "Spare part seller";
		} else {
			return FALSE;
		} */
	...
	private static function instantiate($record) {
		...
		$object = new $class_name; // This is used to make an instantiation of the Class. Its similar to ($business categories = new business categories();)
		// $object->id 			= $record['id'];
	...
	// Create a business categories record and insert it to the database
	...
	// update a business category record in the database
	...
	public function updateColumn($technician, $spare_part_seller) {
	  global $database;
		// Don't forget your SQL syntax and good habits:
		// - UPDATE table SET key='value', key='value' WHERE condition
		// - single-quotes around all values
		...
		$technician = $database->escape_value($technician);
		$spare_part_seller = $database->escape_value($spare_part_seller);
		$sql = "UPDATE ".static::$table_name." SET ";
		$sql .= "{$technician}='". $database->escape_value($this->technician) ."', ";
		$sql .= "{$spare_part_seller}='". $database->escape_value($this->spare_part_seller) ."' ";
	...
	public function delete() {
		...
		// NB: After deleting, the instance of Business_Category still 
		// exists, even though the database entry does not.
		// This can be useful, as in:
		//   echo $businessCategory->first_name . " was deleted";
		// but, for example, we can't call $businessCategory->update() 
		// after calling $businessCategory->delete().
	}
}
