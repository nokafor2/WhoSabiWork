...
class Photograph {
	...
	protected static $db_fields=array('id', 'customers_id', 'filename', 'type', 'size', 'caption', 'visible', 'ad_photo', 'avatar', 'date_created', 'date_edited');
	public $id;
	public $customers_id;
	public $filename;
	public $type;
	...
	public function attach_file($file) {
		...
		if(!$file || empty($file) || !is_array($file)) {
			...
			global $session;
			$cus_username = $session->customer_username;
			$unique_id = uniqid($cus_username.'_'); 
	...
	public function save() {
		...
		if(isset($this->id)) {
			...
			if(move_uploaded_file($this->temp_path, $target_path)) {
				...
				$_SESSION['photo_customers_id'] = $this->customers_id;
				$_SESSION['photo_filename'] = $this->filename;
				$_SESSION['photo_type'] = $this->type;
				$_SESSION['photo_size'] = $this->size;
				$_SESSION['photo_caption'] = $this->caption;
				$_SESSION['photo_ad_photo'] = false;
				$_SESSION['photo_visible'] = true;
				$_SESSION['photo_avatar'] = $this->avatar;
				$_SESSION['photo_date_created'] = current_Date_Time();
	...
	// This will only find the photos that are displayable in the photo gallery including the avatar photo
	public static function find_by_customerId($customers_id=0) {
		global $database;
		$customers_id = $database->escape_value($customers_id);
		
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE customers_id=".$customers_id);
	}

	// Get the record of the customer with its current ad photo id
	public static function find_ad_photo($customers_id=0) {
		global $database;
		$customers_id = $database->escape_value($customers_id);
		
		$result_array = self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE customers_id=".$customers_id." AND ad_photo=1 LIMIT 1");

		return !empty($result_array) ? array_shift($result_array) : false;
	}

	// Get the record of the customer with its current avatar
	public static function find_avatar($customers_id=0) {
		global $database;
		$customers_id = $database->escape_value($customers_id);
		
		$sql = "SELECT * FROM ".self::$table_name." WHERE customers_id=".$customers_id." AND avatar=1 LIMIT 1;";
		$result_array = self::find_by_sql($sql);

		return !empty($result_array) ? array_shift($result_array) : false;
	}
	
	// This will return all the specified customers images that is visible
	public static function find_customer_images($customers_id=0) {
		global $database;
		$customers_id = $database->escape_value($customers_id);
		
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE customers_id=".$customers_id." AND visible=1 AND avatar=0 ORDER BY date_created ASC");
	}

	// This will return all the specified customers images that is visible
	public static function find_customer_images_by_limit($limit, $offset) {
		global $database;
		$limit = $database->escape_value($limit);
		$offset = $database->escape_value($offset);
		
		$sql = "SELECT * FROM ".self::$table_name." WHERE visible=1 AND avatar=0 AND customers_id IN (";
		$sql .= "SELECT customers.id FROM customers WHERE customers.account_status = 'active' AND customers.id IN (";
		$sql .= "SELECT customers_id FROM ".self::$table_name;
		$sql .= ")) ORDER BY date_created DESC LIMIT {$limit} OFFSET {$offset};";

		return self::find_by_sql($sql);
	}

	// This function gets an array of photograph id with the specified array of customer ids
	public function get_photograph_ids($customer_ids) {
		global $database;
	...
	public function getImageNames() {
		$dir_path = SITE_ROOT .DS. 'Public' .DS. $this->upload_dir;
		$imgNameArray = $this->getImagesInDir($dir_path);

		return $imgNameArray;
	}

	public function leftoverImages() {
		// Get photos in the image folder
		$filenames = $this->getImageNames();
		
		// Get all images in the customer photograph table
		$photos = $this->find_all();
		$photoNames = array();
		...
		// merge the customers and users photograph arrays
	...
	public function deleteLeftoverImages() {
		$leftoverImages = $this->leftoverImages();
		$allowedImages = array("emptyImageIcon.png", "globalnav_bg.png", "WhoSabiWorkL1.jpg", "WhoSabiWorkLogo.jpg", "WhoSabiWorkLogo.png");
		$imageDifference = array_diff($leftoverImages, $allowedImages);

		foreach ($imageDifference as $value) {
			$target_path = SITE_ROOT .DS. 'Public' .DS. $this->upload_dir .DS. $value;
			if (file_exists($target_path)) {
				unlink($target_path);
			}
		}
	...
	// Get ad images for desktop resolution 
	public function getDesktopAdImages() {
		$dir_path = SITE_ROOT .DS. 'Public' .DS. $this->upload_dir .DS. 'homePageImages' .DS. 'jpg';
		$imgNameArray = $this->getImagesInDir($dir_path);

		return $imgNameArray;
	}

	// Get ad images for mobile resolution
	public function getMobileAdImages() {
		$dir_path = SITE_ROOT .DS. 'Public' .DS. $this->upload_dir .DS. 'mobileImages' .DS. 'jpg';
		$imgNameArray = $this->getImagesInDir($dir_path);
	...
	public function getImagesInDir($dir_path) {
		if (is_dir($dir_path)) {
			...
			if ($files) {
				...
				while (($file_name = readdir($files)) !== FALSE) {
					if ($file_name != '.' && $file_name != '..') {
						...
						/* if ($this->file_extension($file_name)) {
							// get file extension
							$file_extension = $this->file_extension($file_name);
							// check if file extension is of image type
							if (in_array($file_extension, $this->allowed_extensions)) {
								$imgNameArray[] = $file_name;
							}
						} */
						...
						if (!empty($splitFileName)) {
							// get file extension
							$file_extension = end($splitFileName);
							if (in_array($file_extension, $this->allowed_extensions)) {
								$imgNameArray[] = $file_name;
							}
						}
	...
	// this returns the photograph ids with a passed in customer id, however, if there is a customer without a photo it doesn't identify it
	public function get_photograph_ids_by_customer_ids2($customer_Ids_Array) {
		global $database;
		$customerIds = join(", ", array_values($customer_Ids_Array));

		// This will select a distinct row of the customer's photo record without a duplicate. 
		// Distinct can bus used, but its not flexible as it will only select one column
		$sql = "SELECT id, customers_id FROM ".static::$table_name." WHERE customers_id IN (".$customerIds.") AND visible = 1 AND avatar = 0 GROUP BY customers_id;";
		...
		while($row = mysqli_fetch_assoc($result_set)){ 
			...
			$photographIds[] = $row["id"]; 
		}

		return $photographIds;		
	}

	// this returns the photograph ids with a passed in customer id and it will identify a customer without a photo
	function get_photograph_ids_by_customer_ids($customer_ids) {
		global $database;
		$photograph_ids = array();
		$count = 0;
