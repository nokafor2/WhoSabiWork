<?php
/* 
Bus_Brand Class for Ayuanorama database containing functions to get data from the database.
*/
...
class Bus_Brand {
	...
	protected static $table_name = "bus_brands";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'customers_id','toyota', 'honda', 'nissan', 'mazda', 'mitsubishi', 'suzuki', 'subaru', 'scion', 'kia', 'hyundai', 'acura', 'infinity', 'lexus', 'mercedes_benz', 'BMW', 'volkswagen', 'audi', 'ford', 'chrystler', 'dodge', 'chevrolet', 'GMC', 'peugout', 'renault', 'innoson', 'volvo');
	// Every column in the bus_brand table have an attribute
	...
	// This function returns all the bus_brands from the bus_brands table in a database. It will also return them as an array of objects.
	public static function find_all() {
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM bus_brands"
		return static::find_by_sql("SELECT * FROM ".static::$table_name);
    }
  
    // This function will return a single bus_brand record from the bus_brand table in a database. It will also return them as objects.
    public static function find_by_id($id=0) {
		global $database;
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM bus_brands WHERE id={$id} LIMIT 1"
	...
	public function selected_choices($customerID) {
		...
		// $busTypes = array('toyota', 'honda', 'nissan', 'mazda', 'mitsubishi', 'suzuki', 'subaru', 'scion', 'kia', 'hyundai', 'acura', 'infinity', 'lexus', 'mercedes_benz', 'BMW', 'volkswagen', 'audi', 'ford', 'chrystler', 'dodge', 'chevrolet', 'GMC', 'peugout', 'renault', 'innoson', 'volvo');
		$busTypes = array_keys($this->getBusBrands());
		foreach ($rowData as $key => $value) {
			if (in_array($key, $busTypes) && $value == TRUE) {
	...
	// Create a bus_brand record and insert it to the database
	...
	// update a bus_brand record in the database
	...
	public function delete() {
		...
		// NB: After deleting, the instance of Bus_Brand still 
		// exists, even though the database entry does not.
		// This can be useful, as in:
		//   echo $bus_brand->first_name . " was deleted";
		// but, for example, we can't call $bus_brand->update() 
		// after calling $bus_brand->delete().
	}

	public function busBrandsTypeOptions() {
		// get an array of the busBrands types
		// $busBrands = array_keys($this->getBusBrands());
		
		// Get the available busBrands registered
		$busBrands = $this->getAvailableBusBrands();
		// Sort the array in alphabetical order
		asort($busBrands);
		
		$output  = "";
		$output .= "<option value=''>Select</option>";
		foreach ($busBrands as $busBrands) {
			$output .= "<option value='{$busBrand}'>".ucfirst(str_replace("_", " ", $busBrand))."</option>";
		}
		
		return $output;
	}

	public function getAvailableBusBrands($businessCategory) {
		global $database;
		$businessCategory = $database->escape_value($businessCategory);
		$busBrands = $this->getBusBrands();
		// get an array of the busBr types
		$busBrands = array_keys($busBrands);
		// Generate SQL query to get available busBrands
		...
		$sql .= "SELECT vehicle_categories.customers_id FROM vehicle_categories WHERE vehicle_categories.bus = 1";
		$sql .= ")))";
		$result_set = $database->query($sql);
		
		$selectedBusBrands = array();
		// get the various busBrands in a 2x2 array or table
		while($row = mysqli_fetch_assoc($result_set)){ 
			// Gets the rows of the table 
			$selectedBusBrands[] = $row; 
		}

		$availableBusBrands = array();
		foreach ($busBrands as $key => $value) {
			$selectedColumn = array_column($selectedBusBrands, $value);
			if (in_array(1, $selectedColumn)) {
				$availableBusBrands[] = $value;
			}
		}

		$ucfAvailableBusBrands = array();
		// capitalize the first character of each word
		foreach ($availableBusBrands as $key => $value) {
			$ucfAvailableBusBrands[] = ucfirst($value);
		}
		// Sort the arrays alphabetically in the array
		sort($ucfAvailableBusBrands);
		return $ucfAvailableBusBrands;
	}

	// Get the vehicle brands which are attributes of this class.
	public function getBusBrands() {
		...
		// return alphabetically sorted bus brands;
