<?php
/* 
Photograph_like Class for WhoSabiWork.com database containing functions to get data from the database.
*/

require_once(LIB_PATH.DS.'database.php');

class Photograph_Like {
	
	// This is variable will be used to dynamically choose a table from the database
	protected static $table_name = "photographs_likes";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'photograph_id', 'customers_id', 'public_user_id', 'public_customer_id', 'likes', 'date_created');
	// Every column in the photograph_like table have an attribute
	...
	// This function returns all the photograph_likes from the photographs_likes table in a database. It will also return them as an array of objects.
	public static function find_all() {
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM photographs_likes"
		return static::find_by_sql("SELECT * FROM ".static::$table_name);
    }
  
    // This function will return a single photograph_like record from the photographs_likes table in a database. It will also return them as objects.
    public static function find_by_id($id=0) {
		global $database;
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM photographs_likes WHERE id={$id} LIMIT 1"
    ...
    public static function count_photograph_likes($photographId=0, $customerId=0) {
    	global $database;
			$photographId = $database->escape_value($photographId);
			$customerId = $database->escape_value($customerId);

    	$sql = "SELECT COUNT(*) FROM ".static::$table_name." WHERE photograph_id={$photographId} AND customers_id={$customerId} AND likes = 1";
    ...
    // Find all the likes for a photograph
    public static function find_all_photograph_likes($photographId) {
    	global $database;
		$photographId = $database->escape_value($photographId);
    ...
    public static function find_photograph_liked_by_user($photographId=0, $public_user_id) {
		global $database;
		$photographId = $database->escape_value($photographId);
		$public_user_id = $database->escape_value($public_user_id);
		
		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE photograph_id={$photographId} AND public_user_id={$public_user_id} AND likes=1 LIMIT 1");
		
		return !empty($result_array) ? array_shift($result_array) : false;
    }

	public static function find_photograph_liked_by_customer($photographId=0, $public_customer_id) {
		global $database;
		$photographId = $database->escape_value($photographId);
		$public_customer_id = $database->escape_value($public_customer_id);
		
		$sql = "SELECT * FROM ".static::$table_name." WHERE photograph_id={$photographId} AND public_customer_id={$public_customer_id} AND likes=1 LIMIT 1";
		$result_array = static::find_by_sql($sql);
		return !empty($result_array) ? array_shift($result_array) : false;
    }

    // return an array of all the customers photograph liked by the user id
    public static function find_all_photograph_liked_by_user($public_user_id) {
    	global $database;
		$public_user_id = $database->escape_value($public_user_id);

		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE public_user_id={$public_user_id}");
		
		return $result_array;
    }

    // return an array of all the customers photograph_liked by the customer id
    public static function find_all_photograph_liked_by_customer($public_customer_id) {
    	global $database;
		$public_customer_id = $database->escape_value($public_customer_id);

		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE public_customer_id={$public_customer_id}");
	...
	private static function instantiate($record) {
		...
		$object = new $class_name; // This is used to make an instantiation of the Class. Its similar to ($photographs_likes = new photographs_likes();)
	...
	// Create an photograph_like record and insert it to the database
	...
	// update an photograph_like record in the database
	...
	public function delete() {
		...
		// NB: After deleting, the instance of photograph_like still 
