<?php
/* 
csrf_token Class for WhoSabiWork.com database containing functions to get data from the database.
*/
...
class CSRF_Token {
	...
	protected static $table_name = "csrf_tokens";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'token', 'used', 'date_created');
	// Every column in the csrf_token table have an attribute
	...
	// This function returns all the csrf_tokens from the csrf_tokens table in a database. It will also return them as an array of objects.
	public static function find_all() {
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM csrf_tokens"
		return static::find_by_sql("SELECT * FROM ".static::$table_name);
    }
  
    // This function will return a single csrf_token record from the csrf_tokens table in a database. It will also return them as objects.
    public static function find_by_id($id=0) {
		global $database;
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM csrf_tokens WHERE id={$id} LIMIT 1"
    ...
    // This function will return a single csrf_token record from the csrf_tokens table in a database. It will also return them as objects.
    public static function find_by_token($token=0) {
		global $database;
		// 'self' is replaced with 'static' because of use of late static binding to occur at run time.
		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE token={$token} LIMIT 1");
		// This will return only the first element in the array. Using the array_shift() will convert the array type to its content type, this case it is an object.
		return !empty($result_array) ? array_shift($result_array) : false;
    }
	...
	private static function instantiate($record) {
		...
		$object = new $class_name; // This is used to make an instantiation of the Class. Its similar to ($csrf_tokens = new csrf_tokens();)
	...
	// Create a csrf_token record and insert it to the database
	...
	// update a csrf_token record in the database
	...
	public function delete() {
		...
		// NB: After deleting, the instance of csrf_token still 
