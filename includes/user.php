...
class User {
	...
	protected static $db_fields = array('id', 'customers_id', 'first_name', 'last_name', 'gender', 'username', 'password', 'phone_number', 'phone_validated', 'user_email', 'account_status', 'reset_token', 'fb_user_id', 'fb_access_token', 'google_user_id', 'google_access_token', 'date_created', 'date_edited');
  ...
  public static function find_by_sql($sql="") {
	...
	public function authenticate($loginId="", $loginType="", $user_password="") {
		global $database;
		
		// The values of the loginType and password are refined using the escape_value() function to check it.
		$loginId = $database->escape_value($loginId);
		$loginType = $database->escape_value($loginType);
		$user_password = $database->escape_value($user_password);

		// SQL to get password of user from database
		if ($loginType === 'email') {
			$user = static::find_by_email($loginId);
			$sql = "SELECT * FROM ".static::$table_name." WHERE user_email='{$loginId}' LIMIT 1";
		} elseif ($loginType === 'phone_number') {
			$user = static::find_by_phone_number($loginId);
			$sql = "SELECT * FROM ".static::$table_name." WHERE phone_number='{$loginId}' LIMIT 1";
		} else {
			$user = static::find_by_username($loginId);
		...
		if ($user) {
			...
			if ($this->password_check($user_password, $db_password)) {
				// password matches
				return $user;
			} else {
	...
	// Old authenticate function for just username
	/*
	public function authenticate($username="", $user_password="") {
		global $database;
  ...
  public function full_name() {
	...
	// to delete the token.
	private function set_user_reset_token($username, $token_value) {
		global $database;
		$username = $database->escape_value($username);
	...
	public function token_is_recent($token) {
		...
		$userStatic = static::find_by_column_name('reset_token', $token);
		$max_elapsed = 60 * 60 * 0.5; // Convert 30 minutes to seconds
		$stored_time = strtotime($userStatic->date_edited); 
	...
	public function find_user_with_token($token) {
		...
		if(!$validate->has_presence($token)) {
			...
			$user = static::find_by_column_name('reset_token', $token);
	...
	public function email_reset_token($username) {
		...
		global $session;
		$username = $database->escape_value($username);
		$user = static::find_by_column_name('username', $username);
		...
		if(!empty($user)) {
			...
			if (isset($to)) {
				...
				<p>
					<a href='https://www.whosabiwork.com/Public/resetPassword.php?token=".u($user->reset_token)."&account=user'>https://www.whosabiwork.com/Public/resetPassword.php?token=".u($user->reset_token)."&account=user</a>
				</p>
	...
	/* Find User by the first name or last name or both */
	public function findUserByName($searchVal) {
		global $database;
		...
		if (count($searchArray) > 1) {
			$sql = "SELECT id, first_name, last_name, gender, username, phone_number, phone_validated, user_email, account_status, date_created, date_edited FROM ".static::$table_name." WHERE first_name LIKE '%".$searchArray[0]."%' AND last_name LIKE '%".$searchArray[1]."%';";
		...
		foreach ($searchArray as $key => $value) {
			$sql = "SELECT id, first_name, last_name, gender, username, phone_number, phone_validated, user_email, account_status, date_created, date_edited FROM ".static::$table_name." WHERE first_name LIKE '%".$value."%' OR last_name LIKE '%".$value."%';";
