<?php
/* 
Users_feedback Class for Ayuanorama database containing functions to get data from the database.
*/
...
class Users_Feedback {
	
	// This is variable will be used to dynamically choose a table from the database
	protected static $table_name = "users_feedback";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'first_name', 'last_name', 'phone_number', 'email_address', 'message_subject', 'message_content', 'date_created', 'resolved');
	// Every column in the users_feedback table have an attribute
	...
	// This function returns all the users from the users_feedback table in a database. It will also return them as an array of objects.
	public static function find_all() {
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM users_feedback"
		return static::find_by_sql("SELECT * FROM ".static::$table_name);
  }
  
    // This function will return a single users_feedback record from the users_feedback table in a database. It will also return them as objects.
  public static function find_by_id($id=0) {
		global $database;
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM users_feedback WHERE id={$id} LIMIT 1"
	...
	// This function will return a single users_feedback record from the users_feedback table in a database using the username. It will also return them as objects.
  ...
  public static function find_by_sql($sql="") {
	...
	// This returns the full name of the user from the users_feedback table in the database
  public function full_name() {
	...
	// Create a users_feedback record and insert it to the database
	...
	// update a users_feedback record in the database
	...
	public function delete() {
		...
		// NB: After deleting, the instance of users_feedback still 
		// exists, even though the database entry does not.
		// This can be useful, as in:
		//   echo $users_feedback->first_name . " was deleted";
		// but, for example, we can't call $users_feedback->update() 
		// after calling $users_feedback->delete().
	}

	public function find_by_feedback_subject($subject) {
		global $database;
		$subject = $database->escape_value($subject);

		$sql = "SELECT * FROM ".static::$table_name." WHERE message_subject = '".$subject."'";

		return static::find_by_sql($sql);
	}

	public function count_feedback($subject, $type) {
		global $database;
		if ($type === 'resolved') {
			$resolved = 1;
		} else {
		...
		$sql = "SELECT COUNT(*) FROM ".self::$table_name." WHERE message_subject = '".$subject."' AND resolved = ".$resolved;
		$result_set = $database->query($sql);
		$row = $database->fetch_array($result_set);
		// The single number is returned in a row like an array, so the array_shift function is used to pull out the number.
		return array_shift($row);
