<?php
/* 
Photograph_comment Class for WhoSabiWork.com database containing functions to get data from the database.
*/

require_once(LIB_PATH.DS.'database.php');

class Photograph_Comment {
	
	// This is variable will be used to dynamically choose a table from the database
	protected static $table_name = "photographs_comments";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'photograph_id', 'customers_id', 'public_user_id', 'public_customer_id', 'comment', 'date_created');
	// Every column in the photograph_comment table have an attribute
	...
	public static function make($photographId, $customersId, $userOrCusId, $comment="") {
		global $session;
		
		if(!empty($photographId) && !empty($customersId) && !empty($userOrCusId) && !empty($comment)) {
			// an instance of a class is created in this fuction because the function is a static one. The this keyword can't be used in here. so the created instance will be used to reference the variables in the Photograph_Comment class.
			$photoComment = new Photograph_Comment();
			$photoComment->photograph_id = $photographId;
			$photoComment->customers_id = $customersId;
			if ($session->is_user_logged_in()) {
				$photoComment->public_user_id = $userOrCusId;
			} elseif($session->is_customer_logged_in()) {
				$photoComment->public_customer_id = $userOrCusId;
			}
			$photoComment->date_created = strftime("%Y-%m-%d %H:%M:%S", time());
	...
	// This function returns all the photograph_comments from the photographs_comments table in a database. It will also return them as an array of objects.
	public static function find_all() {
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM photographs_comments"
		return static::find_by_sql("SELECT * FROM ".static::$table_name);
    }
  
    // This function will return a single photograph_comment record from the photographs_comments table in a database. It will also return them as objects.
    public static function find_by_id($id=0) {
		global $database;
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM photographs_comments WHERE id={$id} LIMIT 1"
    ...
    // Get the number of comments for the photograph
    public static function count_photograph_comments($photographId=0, $customerId=0) {
    	global $database;
		$photographId = $database->escape_value($photographId);
		$customerId = $database->escape_value($customerId);

    	$sql = "SELECT COUNT(*) FROM ".static::$table_name." WHERE photograph_id={$photographId} AND customers_id={$customerId}";
    ...
    public static function find_photograph_comments($photographId=0, $customerId) {
		global $database;
		$photographId = $database->escape_value($photographId);
		$customerId = $database->escape_value($customerId);
		
		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE photograph_id={$photographId} AND customers_id={$customerId}  ORDER BY date_created DESC");
		
		return !empty($result_array) ? $result_array : false;
    }

    // Find all the photograph comments using the photograph id
    public static function find_photograph_comments_by_photoId($photographId) {
    	global $database;
		$photographId = $database->escape_value($photographId);
    ...
    public static function find_photograph_commented_by_user($photographId=0, $public_user_id) {
		global $database;
		$photographId = $database->escape_value($photographId);
		$public_user_id = $database->escape_value($public_user_id);
		
		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE photograph_id={$photographId} AND public_user_id={$public_user_id}  ORDER BY date_created DESC");
		
		return !empty($result_array) ? $result_array : false;
    }

	public static function find_photograph_commented_by_customer($photographId=0, $public_customer_id) {
		global $database;
		$photographId = $database->escape_value($photographId);
		$public_customer_id = $database->escape_value($public_customer_id);
		
		$result_array = static::find_by_sql("SELECT * FROM ".static::$table_name." WHERE photograph_id={$photographId} AND public_customer_id={$public_customer_id}  ORDER BY date_created DESC");
	...
	private static function instantiate($record) {
		...
		$object = new $class_name; // This is used to make an instantiation of the Class. Its similar to ($photographs_comments = new photographs_comments();)
	...
	// Create a photograph_comment record and insert it to the database
	...
	// update a photograph_comment record in the database
	...
	public function delete() {
		...
		// NB: After deleting, the instance of photograph_comment still 
