<?php
/* 
Spare part seller Class for Ayuanorama database containing functions to get data from the database.
*/
...
class Spare_Part {
	...
	protected static $table_name = "spare_parts";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'customers_id', 'engine', 'tyre', 'mechanical_part', 'electrical_part', 'mirror_and_door', 'body_part', 'light', 'windscreen', 'upholstery', 'lubricant', 'dashboard', 'bumper', 'air_conditioning_part');
	// Every column in the spare_parts table have an attribute
	// These values gets instantiated when the any of the SQL request is asked.
	public $id;
	public $customers_id;
	public $engine;
	public $tyre;
	public $mechanical_part;
	public $electrical_part;
	public $mirror_and_door;
	public $body_part;
	public $light;
	public $windscreen;
	public $upholstery;
	...
	public $dashboard;
	public $bumper;
	public $air_conditioning_part;
	
	// This function returns all the spare_parts from the spare_parts table in a database. It will also return them as an array of objects.
	public static function find_all() {
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM spare_parts"
		return static::find_by_sql("SELECT * FROM ".static::$table_name);
    }
  
    // This function will return a single spare_parts record from the spare_parts table in a database. It will also return them as objects.
    public static function find_by_id($id=0) {
		global $database;
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM spare_parts WHERE id={$id} LIMIT 1"
	...
	public function selected_choices($customerID) {
		...
		$serviceTypes = array_keys($this->getSpareParts());
	...
	public function getSpareParts() {
		...
		ksort($slicedArray);
		// Return the array keys only which will be the spare_parts
		$spare_parts = array_keys($slicedArray);
		
		// return alphabetically sorted spare parts;
		return array_flip($spare_parts); // Use the array keys so that it will be recovered from json.
	}
	...
	// Create a spare_parts record and insert it to the database
	...
	public function delete() {
		...
		// NB: After deleting, the instance of spare_parts still 
		// exists, even though the database entry does not.
		// This can be useful, as in:
		//   echo $spare_parts->first_name . " was deleted";
		// but, for example, we can't call $spare_parts->update() 
		// after calling $spare_parts->delete().
	}

	public function getAvailableSpareParts($vehicleType, $vehicleBrand, $businessCategory) {
		global $database;
		...
		// get the various spare parts from the database
		$spareParts = $this->getSpareParts();
		// get an array of the spare part types
		$spareParts = array_keys($spareParts);

		// check if its for a car_brand, bus_brand or truck_brand
		if ($vehicleType === 'car') {
		...
		// Generate SQL query to get available spare parts
		...
		// get the various spareParts in a 2x2 array or table
		while($row = mysqli_fetch_assoc($result_set)){ 
			// Gets the rows of the table 
			$selectedSpareParts[] = $row; 
		}

		$availableSpareParts = array();
		foreach ($spareParts as $key => $value) {
			$selectedColumn = array_column($selectedSpareParts, $value);
			if (in_array(1, $selectedColumn)) {
				$availableSpareParts[] = $value;
			}
		}

		$ucfAvailableSpareParts = array();
		// capitalize the first character of each word
		foreach ($availableSpareParts as $key => $value) {
			$ucfAvailableSpareParts[] = ucfirst($value);
		}
		// Sort the arrays alphabetically in the array
		sort($ucfAvailableSpareParts);
		return $ucfAvailableSpareParts;
	}
}
