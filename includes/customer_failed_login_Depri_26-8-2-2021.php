...
class Customer_Failed_Login {
	...
	protected static $db_fields = array('id', 'customer_id', 'customer_username', 'record', 'last_time');
	...
	public function record_customer_failed_login($username) {
		global $database;
		$username = $database->escape_value($username);
		$failed_login = static::find_by_customer_username($username);

		if(empty($failed_login)) {
			// Find an id that matches the username in the customers table in the database
			$customerStatic = Customer::find_by_username($username);
			if (!empty($customerStatic)) {
				// Get the customers id and save it
				$username_id = $customerStatic->id;
			} else {
				$username_id = NULL;
			}
			
			// Create a record of a new failed login
			$this->customer_id = $username_id;
	...
	public function clear_customer_failed_logins($username) {
		global $database;
		$username = $database->escape_value($username);
		$failed_login = static::find_by_customer_username($username);
	...
	public function throttle_failed_customer_logins($username) {
		$throttle_at = 10;
		$delay_in_minutes = 10;
