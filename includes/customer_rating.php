...
class Customer_Rating {
	...
	protected static $table_name = "customers_ratings";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'customers_id', 'rating', 'rated_by_user', 'rated_by_customer', 'ip_address', 'date_rated');
	// Every column in the User table have an attribute
	// These values gets instantiated when the any of the SQL request is asked.
	public $id;
	public $customers_id;
	public $rating;
	public $rated_by_user;
	public $rated_by_customer;
	public $ip_address;
	public $date_rated;
    ...
    // Get the record of a customer using the customer_id
	...
	function get_rating($cus_id) {
		global $database;
		$cus_id = $database->escape_value($cus_id);
		
		$sql = "SELECT customers_ratings.customers_id, AVG(customers_ratings.rating) AS rating FROM ".static::$table_name." WHERE customers_ratings.customers_id = {$cus_id}";
		$result_set = $database->query($sql);
	...
	function getRatingAndCount($cus_id) {
		global $database;
		$cus_id = $database->escape_value($cus_id);
		
		$sql = "SELECT customers_id, AVG(rating) AS rating, COUNT(rating) AS counts FROM ".static::$table_name." WHERE customers_id = {$cus_id}";
		$result_set = $database->query($sql);
		$record = mysqli_fetch_assoc($result_set);
	...
	// This function checks if the user browsing the page has rated the customer before.
	function check_user_rated($cus_id) {
		// $_SESSION["customer_page"] is the id of the customer gotten from customerHomePage $_GET global
		global $database;
		$cus_id = $database->escape_value($cus_id);
		
		$sql = "SELECT rated_by_user, customers_id FROM ".static::$table_name." WHERE customers_id = {$cus_id} AND rated_by_user = {$_SESSION['user_id']}";
		$result_set = $database->query($sql);
		$record_count = 0; 
		while($row = mysqli_fetch_assoc($result_set)){ 
			$record_count++; 
	...
	// This function checks if the customer browsing the page page has rated the customer before.
	function check_customer_rated($cus_id) {
		// $_SESSION["customer_page"] is the id of the customer gotten from customerHomePage $_GET global
		global $database;
		$cus_id = $database->escape_value($cus_id);
		
		$sql = "SELECT rated_by_customer, customers_id FROM ".static::$table_name." WHERE customers_id = {$cus_id} AND rated_by_customer = {$_SESSION['customer_id']}";
		$result_set = $database->query($sql);
		$record_count = 0; 
		while($row = mysqli_fetch_assoc($result_set)){ 
			$record_count++; 
	...
	// This function counts the number of rate for customer/technician.
	public function rate_count($cus_id) {
		// $_SESSION["customer_page"] is the id of the customer gotten from customerHomePage $_GET global
		global $database;
		$cus_id = $database->escape_value($cus_id);
		
		$sql = "SELECT DISTINCT rated_by_user, rated_by_customer FROM customers_ratings WHERE customers_id = {$cus_id}";
		$result_set = $database->query($sql);
		$rate_count = 0; 
		while($row = mysqli_fetch_assoc($result_set)){ 
			$rate_count++; 
