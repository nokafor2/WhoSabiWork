...
class User_Failed_Login {
	...
	protected static $db_fields = array('id', 'user_id', 'username', 'email', 'phone_number', 'record', 'last_time');
  ...
  public static function find_by_sql($sql="") {
	...
	public function record_user_failed_login($loginId) {
		...
		if ($loginIdType === 'username') {
			...
			if (!empty($userStatic)) {
				$failed_login = $this->getFailedLoginRecord($userStatic, $loginId, $loginIdType); 

				if (empty($failed_login)) {
					// Create a record of a new failed login
					$this->user_id = $userStatic->id;
					$this->username = $userStatic->username;
					$this->email = NuLL;
					...
					if ($failed_login->record > $this->throttle_at) {				
						...
						$failed_login->username = $userStatic->username;
				...
				$failed_login = User_Failed_Login::find_by_column_name('username', $loginId);
				if (empty($failed_login)) {
			...
			if (!empty($userStatic)) {
				...
				if (empty($failed_login)) {
					...
					if ($failed_login->record > $this->throttle_at) {				
						...
						$failed_login->email = $userStatic->user_email;
						$failed_login->phone_number = NuLL;
						$failed_login->record = 1; 
						$failed_login->last_time = $failed_login->current_Date_Time();
						$failed_login->update();
			...
			if (!empty($userStatic)) {
				...
				if (empty($failed_login)) {
					...
					if ($failed_login->record > $this->throttle_at) {				
						...
						$failed_login->username = NuLL;
						$failed_login->email = NuLL;
						$failed_login->phone_number = $userStatic->phone_number;
						$failed_login->record = 1; 
						$failed_login->last_time = $failed_login->current_Date_Time();
						$failed_login->update();
					} else {
				...
				$failed_login = User_Failed_Login::find_by_column_name('phone_number', $loginId);
				if (empty($failed_login)) {
					// Create a new record for a username that doesn't exist in the User table
					$this->user_id = NuLL;
					$this->username = NuLL;
	...
	public function getFailedLoginRecord($userStatic, $loginId, $loginIdType) {
		// Get an array of username, email and phone_number
		if ($loginIdType === 'username') {
