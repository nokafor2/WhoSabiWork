<?php
/* 
Truck_Brand Class for Ayuanorama database containing functions to get data from the database.
*/
...
class Truck_Brand {
	...
	protected static $table_name = "truck_brands";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'customers_id','sinotruk', 'mack', 'MAN', 'mercedes_benz', 'HOWO', 'volvo', 'tata', 'shacman', 'volkswagen', 'scania', 'renault', 'DAF');
	// Every column in the truck_brand table have an attribute
	...
	// This function returns all the truck_brands from the truck_brands table in a database. It will also return them as an array of objects.
	public static function find_all() {
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM truck_brands"
		return static::find_by_sql("SELECT * FROM ".static::$table_name);
    }
  
    // This function will return a single truck_brand record from the truck_brand table in a database. It will also return them as objects.
    public static function find_by_id($id=0) {
		global $database;
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM truck_brands WHERE id={$id} LIMIT 1"
	...
	public function selected_choices($customerID) {
		...
		// $truckTypes = array("sinotruk", "mack", "MAN", "mercedes_benz", "HOWO", "volvo", "tata", "shacman", "volkswagen", "scania", "renault", "DAF");
		$truckTypes = array_keys($this->getTruckBrands());
		foreach ($rowData as $key => $value) {
			if (in_array($key, $truckTypes) && $value == TRUE) {
	...
	// Create a truck_brand record and insert it to the database
	...
	// update a truck_brand record in the database
	...
	public function delete() {
		...
		// NB: After deleting, the instance of Truck_Brand still 
		// exists, even though the database entry does not.
		// This can be useful, as in:
		//   echo $truck_brand->first_name . " was deleted";
		// but, for example, we can't call $truck_brand->update() 
		// after calling $truck_brand->delete().
	}
	...
	public function getTruckBrands() {
		...
		// return alphabetically sorted truck brands;
		return array_flip($vehicle_brands); // Use the array keys so that it will be recovered from json.
	}

	public function truckBrandsTypeOptions() {
		// get an array of the truckBrands types
		// $truckBrands = array_keys($this->getTruckBrands());
		
		// Get the available truckBrands registered
		$truckBrands = $this->getAvailableTruckBrands();
		// Sort the array in alphabetical order
		asort($truckBrands);
		
		$output  = "";
		$output .= "<option value=''>Select</option>";
		foreach ($truckBrands as $truckBrands) {
			$output .= "<option value='{$truckBrand}'>".ucfirst(str_replace("_", " ", $truckBrand))."</option>";
		}
		
		return $output;
	}

	public function getAvailableTruckBrands($businessCategory) {
		global $database;
		$truckBrands = $this->getTruckBrands();
		// get an array of the truckBr types
		$truckBrands = array_keys($truckBrands);
		// Generate SQL query to get available truckBrands
		...
		$sql .= "SELECT vehicle_categories.customers_id FROM vehicle_categories WHERE vehicle_categories.truck = 1";
		$sql .= ")))";
		$result_set = $database->query($sql);
		
		$selectedTruckBrands = array();
		// get the various truckBrands in a 2x2 array or table
		while($row = mysqli_fetch_assoc($result_set)){ 
			// Gets the rows of the table 
			$selectedTruckBrands[] = $row; 
		}

		$availableTruckBrands = array();
		foreach ($truckBrands as $key => $value) {
			$selectedColumn = array_column($selectedTruckBrands, $value);
			if (in_array(1, $selectedColumn)) {
				$availableTruckBrands[] = $value;
			}
		}

		$ucfAvailableTruckBrands = array();
		// capitalize the first character of each word
		foreach ($availableTruckBrands as $key => $value) {
			$ucfAvailableTruckBrands[] = ucfirst($value);
		}
		// Sort the arrays alphabetically in the array
		sort($ucfAvailableTruckBrands);
		return $ucfAvailableTruckBrands;
	}
}
