<?php
/* 
State and town Class for whoSabiWork database containing functions to get data from the database.
*/
...
class State_Town {
	...
	protected static $table_name = "states_towns";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'abia', 'abuja', 'adamawa', 'akwa_ibom', 'anambra', 'bauchi', 'bayelsa', 'benue', 'borno', 'cross_river', 'delta', 'ebonyi', 'edo', 'ekiti', 'enugu', 'gombe', 'imo', 'jigawa', 'kaduna', 'kano', 'katsina', 'kebbi', 'kogi', 'kwara', 'lagos', 'nasarawa', 'niger', 'ogun', 'ondo', 'osun', 'oyo', 'plateau', 'rivers', 'sokoto', 'taraba', 'yobe', 'zamfara');
	// Every column in the state_town table have an attribute
	// These values gets instantiated when the any of the SQL request is asked.
	public $id, $abia, $abuja, $adamawa, $akwa_ibom, $anambra, $bauchi, $bayelsa, $benue, $borno, $cross_river, $delta, $ebonyi, $edo, $ekiti, $enugu, $gombe, $imo, $jigawa, $kaduna, $kano, $katsina, $kebbi, $kogi, $kwara, $lagos, $nasarawa, $niger, $ogun, $ondo, $osun, $oyo, $plateau, $rivers, $sokoto, $taraba, $yobe, $zamfara;
	
	// This function returns all the state_towns from the state_towns table in a database. It will also return them as an array of objects.
	public static function find_all() {
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM state_towns"
		return static::find_by_sql("SELECT * FROM ".static::$table_name);
    }
  
    // This function will return a single state_town record from the state_town table in a database. It will also return them as objects.
    public static function find_by_id($id=0) {
		global $database;
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM state_towns WHERE id={$id} LIMIT 1"
    ...
    public function addNewTown($state, $newTown) {
    	global $database;
		$state = $database->escape_value($state);
		$newTown = $database->escape_value($newTown);

		// Check if the the town already exist in the state
		// get the towns
		$towns = static::find_by_column($state);
		...
		// convert the new town to lower case
		$newTown = strtolower($newTown);
		// check if the string is not in the array
		if (in_array($newTown, $towns)) {
			// Town already exists in the database
		} else {
			// If its not in the array save it
			// Get the total number of rows in the table
			$rowNumbers = static::count_all();
			// Get the total number of variables in the state column
			$numTowns = static::count_column($state);

			if ($numTowns < $rowNumbers) {
				// update the row
				$stateTown = static::find_by_id($numTowns + 1);
				$stateTown->{$state} = ucwords($newTown);
				$stateTown->update();
			} else {
				// save a new record
				$this->{$state} = ucwords($newTown);
				$this->save();
			}
		}
    ...
    // Get the towns within the states in the column of the table
    // This will return the number of saved values in the column
	public static function find_by_column($columnName) {
		global $database;
		$columnName = $database->escape_value($columnName);
		$sql = "SELECT ".$columnName." FROM ".self::$table_name." WHERE ".$columnName." != ''";
		$result_set = $database->query($sql);
		// $row = $database->fetch_array($result_set);

		$towns = array();
		while($row = mysqli_fetch_assoc($result_set)){ 
			$towns[] = $row[$columnName]; 
		}
		return $towns;
	}
	...
	// This will only return the number of rows of the table
	...
	// This will return the number of saved values in the column
	public static function count_column($columnName) {
		global $database;
		$columnName = $database->escape_value($columnName);
		$sql = "SELECT COUNT(".$columnName.") FROM ".self::$table_name." WHERE ".$columnName." != ''";
		$result_set = $database->query($sql);
		$row = $database->fetch_array($result_set);
		// The single number is returned in a row like an array, so the array_shift function is used to pull out the number.
		return array_shift($row);
	...
	private static function instantiate($record) {
		...
		$object = new $class_name; // This is used to make an instantiation of the Class. Its similar to ($state_town = new State_Town();)
		// $object->id 			= $record['id'];
	...
	// Create a State_Town record and insert it to the database
	...
	// update a State_Town record in the database
