<?php
/* 
Blacklisted Ip Class for Whosabiwork database containing functions to get data from the database.
*/
...
class Blacklisted_Ip {
	...
	protected static $table_name = "blacklisted_ips";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'ip', 'date_created', 'date_edited');
	// Every column in the blacklisted_ip table have an attribute
	...
	// This function returns all the blacklisted_ips from the blacklisted_ips table in a database. It will also return them as an array of objects.
	public static function find_all() {
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM blacklisted_ips"
		return static::find_by_sql("SELECT * FROM ".static::$table_name);
    }
  
    // This function will return a single blacklisted_ip record from the blacklisted_ip table in a database. It will also return them as objects.
    public static function find_by_id($id=0) {
		global $database;
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM blacklisted_ips WHERE id={$id} LIMIT 1"
	...
	public static function find_by_ip($ip) {
		...
		// Previous SQL: "SELECT * FROM blacklisted_ips WHERE id={$id} LIMIT 1"
		// 'self' is replaced with 'static' because of use of late static binding to occur at run time.
		$sql = "SELECT * FROM ".static::$table_name." WHERE ip='{$ip}' LIMIT 1";
		// echo $sql;
	...
	private static function instantiate($record) {
		...
		$object = new $class_name; // This is used to make an instantiation of the Class. Its similar to ($blacklisted_ip = new blacklisted_ip();)
	...
	// Create a blacklisted_ip record and insert it to the database
	...
	// update a blacklisted_ip record in the database
	...
	private function is_blacklisted_ip($ip) {
		global $database;
		$ip = $database->escape_value($ip);
		$blacklisted_ip = static::find_by_ip($ip);
		
		if (isset($blacklisted_ip) && !empty($blacklisted_ip)) {
	...
	public function add_ip_to_blacklist($ip) {
		...
		// $this->date_edited = $this->current_Date_Time();
		if ($this->create()) {
			return true;
		} else {
			return false;
	...
	// add the current user ip to the database if his acts are malicious
