<?php
/* 
Artisan Class for Ayuanorama database containing functions to get data from the database.
*/
...
class Artisan {
	...
	protected static $table_name = "artisans";
	// This is a list of all fields that are database fields.
	protected static $db_fields = array('id', 'customers_id', 'caterer', 'confectioner', 'event_planner', 'make_up_artist', 'tailor', 'jeweller', 'artist', 'tiler', 'horticulturist', 'driver', 'cleaner', 'mover', 'transporter', 'upholsterer', 'shoemaker', 'watchmaker', 'bag_maker', 'milliner', 'tanner', 'fabric_maker','brewer', 'detergent_maker', 'locksmith', 'welder', 'electronics_technician', 'bookbinder', 'carbinet_maker', 'gasman', 'glassblower', 'goldsmith', 'smith', 'mason', 'plumber', 'electrician', 'painter', 'air_condition_technician', 'POP_maker', 'carpenter', 'security_guard', 'glazier', 'drycleaner', 'hair_stylist_and_wig_maker', 'photographer', 'computer_technician', 'phone_technician', 'solar_energy_technician', 'CCTV_technician', 'video_editor', 'barber', 'interior_designer', 'generator_technician', 'DSTV_technician', 'workout_trainer', 'alumaco_fabrication', 'fabric_printer', 'delivery_agent', 'diary_producer', 'cold_room_services', 'real_estate_agent');
	// Every column in the artisan table have an attribute
	// These values gets instantiated when the any of the SQL request is asked.
	public $id, $customers_id;
	public $caterer, $confectioner, $event_planner, $make_up_artist, $tailor, $jeweller, $artist, $tiler, $horticulturist, $driver, $cleaner, $mover, $transporter, $upholsterer, $shoemaker, $watchmaker, $bag_maker, $milliner, $tanner, $fabric_maker, $brewer, $detergent_maker, $locksmith, $welder, $electronics_technician, $bookbinder, $carbinet_maker, $gasman, $glassblower, $goldsmith, $smith, $mason, $plumber, $electrician, $painter, $air_condition_technician, $POP_maker, $carpenter, $security_guard, $glazier, $drycleaner, $hair_stylist_and_wig_maker, $photographer, $computer_technician, $phone_technician, $solar_energy_technician, $CCTV_technician, $video_editor, $barber, $interior_designer, $generator_technician, $DSTV_technician, $workout_trainer, $alumaco_fabrication, $fabric_printer, $delivery_agent, $diary_producer, $cold_room_services, $real_estate_agent;
	
	// This function returns all the artisans from the artisans table in a database. It will also return them as an array of objects.
	public static function find_all() {
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM artisans"
		return static::find_by_sql("SELECT * FROM ".static::$table_name);
  }
  
  // This function will return a single artisan record from the artisan table in a database. It will also return them as objects.
  public static function find_by_id($id=0) {
		global $database;
		// The 'table_name' is used to dynamically choose a table from the database
		// Previous SQL: "SELECT * FROM artisans WHERE id={$id} LIMIT 1"
  ...
  // check for empty ids that has not been saved into
	...
	public function selected_choices($customerID) {
		...
		$artisans = array_keys($this->getArtisans());
		foreach ($rowData as $key => $value) {
			if (in_array($key, $artisans) && $value == TRUE) {
				$selectedChoices[ucfirst(str_replace("_", " ", $key))] = $value;
			}
		}
		ksort($selectedChoices);
  ...
  public static function find_by_sql($sql="") {
	...
	// Get the artisans which are attributes of this class.
	public function getArtisans() {
		...
		// Sort the artisans alphabetically
		ksort($slicedArray);

		// Return the array keys only which will be the artisans
		$artisans = array_keys($slicedArray);
	...
	// Create a artisan record and insert it to the database
	...
	// update a artisan record in the database
	...
	public function delete() {
		...
		// NB: After deleting, the instance of artisan still 
		// exists, even though the database entry does not.
		// This can be useful, as in:
		//   echo $artisan->first_name . " was deleted";
		// but, for example, we can't call $artisan->update() 
		// after calling $artisan->delete().
	}
	
	function artisansTypeOptions() {
		// get an array of the artisan types
		// $artisans = array_keys($this->getArtisans());
		
		// Get the available artisans registered
		$artisans = $this->getAvailableArtisans(); 
		...
		foreach ($artisans as $artisan) {
			$output .= "<option value='{$artisan}'>".ucfirst(str_replace("_", " ", $artisan))."</option>";
		}
		
		return $output;
	}

	function getAvailableArtisans() {
		global $database;
		$artisans = $this->getArtisans();
		// get an array of the artisan types
		$artisans = array_keys($artisans);
		// Generate SQL query to get available artisans
		// $sql = "SELECT ";
		...
		$sql .= "SELECT customers_id FROM business_categories WHERE business_categories.artisan = 1 AND business_categories.customers_id IN (";
		$sql .= "SELECT customers_id FROM ".static::$table_name.")))";
		
		$result_set = $database->query($sql);
		
		$selectedArtisans = array();
		// get the various artisans in a 2x2 array or table
		while($row = mysqli_fetch_assoc($result_set)){ 
		...
		$ucfAvailableArtisans = array();
		// capitalize the first character of each word
		foreach ($availableArtisans as $key => $value) {
			$ucfAvailableArtisans[] = ucfirst($value);
		}
		// Sort the arrays alphabetically in the array
		sort($ucfAvailableArtisans);
		return $ucfAvailableArtisans;
	}
}
