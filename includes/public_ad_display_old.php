...
Class Public_Ad_Display {
	...
	function count_customer_ids($vehicle_name = '', $service_type = '', $cus_category = '', $vehicleType = '', $state = '', $town = ''){
		...
		if (empty($town)) {
			...
			$sql = "SELECT {$vehicleBrand}.customers_id, {$vehicleBrand}.{$vehicle_name}, vehicle_categories.{$vehicleType}, {$techOrParts}.{$service_type}, business_categories.{$cus_category}, customers.account_status, addresses.state FROM {$vehicleBrand}, vehicle_categories, {$techOrParts}, business_categories, customers WHERE {$vehicleBrand}.customers_id = {$techOrParts}.customers_id AND {$vehicleBrand}.customers_id = vehicle_categories.customers_id AND {$vehicleBrand}.customers_id = business_categories.customers_id AND {$vehicleBrand}.customers_id = customers.id AND {$vehicleBrand}.{$vehicle_name} = 1 AND vehicle_categories.{$vehicleType} = 1 AND {$techOrParts}.{$service_type} = 1 AND business_categories.{$cus_category} = 1 AND customers.account_status = 'demo' AND addresses.state = '{$state}';";
		} else {
			// STEP1: generate and parse the SQL command 
			$sql = "SELECT {$vehicleBrand}.customers_id, {$vehicleBrand}.{$vehicle_name}, vehicle_categories.{$vehicleType}, {$techOrParts}.{$service_type}, business_categories.{$cus_category}, customers.account_status, addresses.state, addresses.town FROM {$vehicleBrand}, vehicle_categories, {$techOrParts}, business_categories, customers, addresses WHERE {$vehicleBrand}.customers_id = {$techOrParts}.customers_id AND {$vehicleBrand}.customers_id = vehicle_categories.customers_id AND {$vehicleBrand}.customers_id = business_categories.customers_id AND {$vehicleBrand}.customers_id = customers.id AND {$vehicleBrand}.customers_id = addresses.customers_id AND {$vehicleBrand}.{$vehicle_name} = 1 AND vehicle_categories.{$vehicleType} = 1 AND {$techOrParts}.{$service_type} = 1 AND business_categories.{$cus_category} = 1 AND customers.account_status = 'demo' AND addresses.state = '{$state}' AND addresses.town = '{$town}';";
		}
		
		// SELECT car_brands.customers_id, car_brands.toyota, vehicle_categories.bus, spare_part_sellers.engine, business_categories.spare_part_seller, customers.account_status, addresses.state FROM car_brands, vehicle_categories, spare_part_sellers, business_categories, customers, addresses WHERE car_brands.customers_id = spare_part_sellers.customers_id AND car_brands.customers_id = vehicle_categories.customers_id AND car_brands.customers_id = business_categories.customers_id AND car_brands.customers_id = customers.id AND car_brands.customers_id = addresses.customers_id AND car_brands.toyota = 1 AND vehicle_categories.bus = 1 AND spare_part_sellers.engine = 1 AND business_categories.technician = 1 AND customers.account_status = 'demo' AND addresses.state = 'abuja'
		
		// SELECT car_brands.customers_id, car_brands.toyota, vehicle_categories.bus, technical_services.engine_service, business_categories.technician, customers.account_status, addresses.state FROM car_brands, vehicle_categories, technical_services, business_categories, customers, addresses WHERE car_brands.customers_id = technical_services.customers_id AND car_brands.customers_id = vehicle_categories.customers_id AND car_brands.customers_id = business_categories.customers_id AND car_brands.customers_id = customers.id AND car_brands.customers_id = addresses.customers_id AND car_brands.toyota = 1 AND vehicle_categories.bus = 1 AND technical_services.engine_service = 1 AND business_categories.technician = 1 AND customers.account_status = 'demo' AND addresses.state = 'abuja'
	...
	function get_customer_ids($vehicle_name = '', $service_type = '', $cus_category = '', $vehicleType = '', $state = '', $town = ''){
		...
		if (empty($town)) {
			...
			// STEP1: generate and parse the SQL command 
			// $sql = "SELECT {$vehicleBrand}.customers_id, {$vehicleBrand}.{$vehicle_name}, vehicle_categories.{$vehicleType}, technical_services.{$service_type}, business_categories.{$cus_category}, customers.account_status FROM {$vehicleBrand}, vehicle_categories, technical_services, business_categories, customers WHERE {$vehicleBrand}.customers_id = technical_services.customers_id AND {$vehicleBrand}.customers_id = vehicle_categories.customers_id AND {$vehicleBrand}.customers_id = business_categories.customers_id AND {$vehicleBrand}.customers_id = customers.id  AND {$vehicleBrand}.{$vehicle_name} = 1 AND vehicle_categories.{$vehicleType} = 1 AND technical_services.{$service_type} = 1 AND business_categories.{$cus_category} = 1 AND customers.account_status = 'demo' LIMIT {$per_page} OFFSET {$pagination->offset()}";
			
			$sql = "SELECT {$vehicleBrand}.customers_id, {$vehicleBrand}.{$vehicle_name}, vehicle_categories.{$vehicleType}, {$techOrParts}.{$service_type}, business_categories.{$cus_category}, customers.account_status, addresses.state FROM {$vehicleBrand}, vehicle_categories, {$techOrParts}, business_categories, customers WHERE {$vehicleBrand}.customers_id = {$techOrParts}.customers_id AND {$vehicleBrand}.customers_id = vehicle_categories.customers_id AND {$vehicleBrand}.customers_id = business_categories.customers_id AND {$vehicleBrand}.customers_id = customers.id AND {$vehicleBrand}.{$vehicle_name} = 1 AND vehicle_categories.{$vehicleType} = 1 AND {$techOrParts}.{$service_type} = 1 AND business_categories.{$cus_category} = 1 AND customers.account_status = 'demo' AND addresses.state = '{$state}' LIMIT {$per_page} OFFSET {$pagination->offset()};";
		} else {
			// STEP1: generate and parse the SQL command 
			// $sql = "SELECT {$vehicleBrand}.customers_id, {$vehicleBrand}.{$vehicle_name}, vehicle_categories.{$vehicleType}, technical_services.{$service_type}, business_categories.{$cus_category}, customers.account_status, addresses.state FROM {$vehicleBrand}, vehicle_categories, technical_services, business_categories, customers, addresses WHERE {$vehicleBrand}.customers_id = technical_services.customers_id AND {$vehicleBrand}.customers_id = vehicle_categories.customers_id AND {$vehicleBrand}.customers_id = business_categories.customers_id AND {$vehicleBrand}.customers_id = customers.id  AND {$vehicleBrand}.customers_id = addresses.customers_id AND {$vehicleBrand}.{$vehicle_name} = 1 AND vehicle_categories.{$vehicleType} = 1 AND technical_services.{$service_type} = 1 AND business_categories.{$cus_category} = 1 AND customers.account_status = 'demo' AND addresses.state = '{$state}' LIMIT {$per_page} OFFSET {$pagination->offset()}";
			
			$sql = "SELECT {$vehicleBrand}.customers_id, {$vehicleBrand}.{$vehicle_name}, vehicle_categories.{$vehicleType}, {$techOrParts}.{$service_type}, business_categories.{$cus_category}, customers.account_status, addresses.state FROM {$vehicleBrand}, vehicle_categories, {$techOrParts}, business_categories, customers, addresses WHERE {$vehicleBrand}.customers_id = {$techOrParts}.customers_id AND {$vehicleBrand}.customers_id = vehicle_categories.customers_id AND {$vehicleBrand}.customers_id = business_categories.customers_id AND {$vehicleBrand}.customers_id = customers.id AND {$vehicleBrand}.customers_id = addresses.customers_id AND {$vehicleBrand}.{$vehicle_name} = 1 AND vehicle_categories.{$vehicleType} = 1 AND {$techOrParts}.{$service_type} = 1 AND business_categories.{$cus_category} = 1 AND customers.account_status = 'demo' AND addresses.state = '{$state}' AND addresses.town = '{$town}' LIMIT {$per_page} OFFSET {$pagination->offset()};";
		}
		// echo "The SQL query is: ".$sql."<br/>";
		
		// SELECT car_brands.customers_id, car_brands.toyota, vehicle_categories.bus, technical_services.engine_service, business_categories.technician, customers.account_status, addresses.state, addresses.town FROM car_brands, vehicle_categories, technical_services, business_categories, customers, addresses WHERE car_brands.customers_id = technical_services.customers_id AND car_brands.customers_id = vehicle_categories.customers_id AND car_brands.customers_id = business_categories.customers_id AND car_brands.customers_id = customers.id AND car_brands.customers_id = addresses.customers_id AND car_brands.toyota = 1 AND vehicle_categories.bus = 1 AND technical_services.engine_service = 1 AND business_categories.technician = 1 AND customers.account_status = 'demo' AND addresses.state = 'abuja' AND addresses.town = 'apo'
		
		// SELECT car_brands.customers_id, car_brands.toyota, vehicle_categories.bus, spare_part_sellers.engine, business_categories.spare_part_seller, customers.account_status, addresses.state, addresses.town FROM car_brands, vehicle_categories, spare_part_sellers, business_categories, customers, addresses WHERE car_brands.customers_id = spare_part_sellers.customers_id AND car_brands.customers_id = vehicle_categories.customers_id AND car_brands.customers_id = business_categories.customers_id AND car_brands.customers_id = customers.id AND car_brands.customers_id = addresses.customers_id AND car_brands.toyota = 1 AND vehicle_categories.bus = 1 AND spare_part_sellers.engine = 1 AND business_categories.technician = 1 AND customers.account_status = 'demo' AND addresses.state = 'abuja' AND addresses.town = 'apo'
	...
	function get_artisan_ids($artisanType = '', $state = '', $town = ''){
		...
		$state = $database->escape_value($state);
		$town = $database->escape_value($town);
		// echo "The town is: ".$town;
		$_SESSION['townselect'] = $town;
		if (empty($town)) {
			// STEP1: generate and parse the SQL command 
			$sql = "SELECT artisans.customers_id, business_categories.artisan, customers.account_status, addresses.state FROM artisans, business_categories, customers WHERE artisans.customers_id = business_categories.customers_id AND artisans.customers_id = customers.id AND artisans.{$artisanType} = 1 AND business_categories.artisan = 1 AND customers.account_status = 'demo' AND addresses.state = '{$state}' LIMIT {$per_page} OFFSET {$pagination->offset()};";
		} else {
		...
		echo '<div id="pagination" style="clear:both;">';
			...
			if($pagination->total_pages() > 1) {
				...
				if($pagination->has_previous_page()) { 
					...
					// <a href='servicePageTrial2.php' onlick='display_Technician_Ads(previouspage);' >&laquo; Previous</a>
					// <input type="button" value="Previous" name="previousBtn" id="previousBtn" onclick="display_Technician_Ads(previouspage);"/>
				}
