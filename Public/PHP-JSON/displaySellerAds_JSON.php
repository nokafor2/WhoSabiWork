...
if(request_is_post() && request_is_same_domain()) {
	...
	$post_params = allowed_post_params(['sellerType', 'state', 'town', 'page', 'searchVal', 'searchOrigin', 'searchPage']);
	
	$jsonData = array();
	if (isset($_POST['sellerType'], $_POST['state'], $_POST['town'], $_POST['page'])) {
		$sellerType = $_SESSION['searchedSeller'] = $_POST['sellerType'];
		...
		$selectError = array();
		if (empty($sellerType)) {
			$selectError['sellerType'] = 'noSelectedSeller';
		} else {
			// This is set so that a variable will be sent in the JSON data to the Javascript else an error will be outputted.
			$selectError['sellerType'] = 'yesSelectedSeller';
			// echo "Seller was selected.";
		}
		
		/* $sellerChooseError = array();
		if (empty($sellerType)) {
			$sellerChooseError['sellerType'] = 'noSelectedSeller';
		} else {
			// This is set so that a variable will be sent in the JSON data to the Javascript else an error will be outputted.
			$sellerChooseError['sellerType'] = 'yesSelectedSeller';
			// echo "Seller was selected.";
		} */
		
		if ( $selectError['sellerType'] == 'yesSelectedSeller' ) {
			// echo "There is seller selected.";
			// 2. records per page ($per_page)
			$per_page = 9; // changed from 3
			$displayPublicImage = new Public_Ad_Display($page, $per_page);
			
			// Get the number of sellers
			$numberOfCustomers = $displayPublicImage->count_seller_ids($sellerType, $state, $town);
			// print_r($numberOfCustomers);
			// Set up the pagination object
			$pagination = new Pagination($page, $per_page, $numberOfCustomers);
			// Get the sellers ids which is the customer ids
			list($customer_ids, $total_count) = $displayPublicImage->get_seller_ids($sellerType, $state, $town);
			...
			if (count($customer_ids) < 1) {
				$jsonData = array("available" => "noAvailSeller");
			} else {
				for($i = 0; $i < count($customer_ids); $i++){
					...
					if (isset($techId)) { 
						...
						$full_name = 'Seller name not yet set';
						$phone_number = 'Seller phone number not yet set';
					}
					...
					*/					
					...
					// Concatenate the array of the sellers details retrieved from the database
			...
			$jsonData = array("selectErrors" => $selectError['sellerType']);
		}
