...
$(document).ready(function(){
	...
	$('#submitPhoto').on('click', function(event){
		...
		if (!validationResult1.errorType) {
			...
			return false;
		} else if ((commentValid !== "") && (!commentValid)) {
			$("#imgErrorReport").empty();
		      ...
		      return myXhr;
				...
				success: function(response) {
					...
					if (response.success === "image not saved") {
						...
						// After sucessfully uploading the image
						// Put a default image in place of the image viewer
    				$("#img_show").attr('src', "../images/emptyImageIcon.png");
					}
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data.success) {
					...
					$('.CusPhotoGallery').append("<div id='displayPicture"+imageCounter+"' class='displayPicture'><div name='cus-ad-image' id='cus-ad-image"+imageCounter+"' class='cus-ad-image' style='background: url("+croppedImgPath+"); background-repeat: no-repeat; background-size: cover; background-position: center;' imgurl='"+croppedImgPath+"' onclick='galleryImg(this);' ></div> <div><p id='imageCaption"+imageCounter+"' class='imageCaption'>"+data.photo_caption+"</p><input type='hidden' name='customerId' class='customerId' value='"+customerId+"' /> <input type='hidden' name='imageId' id='imageId"+imageCounter+"' value='"+data.photoId+"' /> </div> </div>");
					$('#display').html('');
		      ...
		      return myXhr;
				...
				success: function(response) {
					...
					if (response.success === "image not saved") {
						...
						// After sucessfully uploading the image
						// Put a default image in place of the image viewer
						$("#avatar_show").attr('src', "../images/emptyImageIcon.png");
					}
...
function validateImageUpload(fileChooser, imgHolderId) {
	...
	// Check if a file was sucessfully read 
	if (fileUploadPath == '') {
		// Put a default image in place of the image viewer
    ...
    // check if the file extension is an allowed image file
