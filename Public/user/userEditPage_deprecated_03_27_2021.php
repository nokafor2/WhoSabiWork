...
if(request_is_get() ) {	
	...
	$showTokenInputField = false;
	$_SESSION['showTokenInputField'] = false;
...
// Check if the request is post and is from same web page.
	...
	if(false) {
		...
		if (isset($post_params['submit_first_name'])) {
			...
			if (empty($validate->errors)) {
				...
				if(isset($savedUser)) {
					...
					$session->message("Your First name was successfully updated.");
					// $message = "First name was successfully updated in the database.";
					redirect_to('userEditPage.php?id='.urlencode($session->user_id));
				} else {
			...
			if (empty($validate->errors)) {
				...
				if(isset($savedUser)) {
					// This message should be saved in the session.
					$session->message("Your last name was successfully updated.");
					redirect_to('userEditPage.php?id='.urlencode($session->user_id));
				} else {
					$message = "An error occurred while saving.";
				}
			...
			if (empty($validate->errors)) {
				...
				if(isset($savedUser)) {
					// This message should be saved in the session.
					$session->message("Your username was successfully updated.");
					redirect_to('userEditPage.php?id='.urlencode($session->user_id));
				} else {
					$message = "An error occurred while saving.";
				}
		...
		} elseif (isset($post_params['submit_email'])) {
			...
			if (empty($validate->errors)) {
				...
				if(isset($savedUser)) {
					// This message should be saved in the session.
					$session->message("Your email was successfully updated.");
					redirect_to('userEditPage.php?id='.urlencode($session->user_id));
				} else {
					$message = "An error occurred while saving.";
				}
			...
			if (empty($validate->errors)) {
				...
				if(isset($savedUser)) {
					// This message should be saved in the session.
					$session->message("Your phone number was successfully updated.");
					// $message = "Phone number was successfully updated in the database.";
					redirect_to('userEditPage.php?id='.urlencode($session->user_id));
				} else {
					$message = "An error occurred while saving.";
				}
		...
		if (isset($post_params['verifyNumber'])) {
			...
			if (sendSMSCode($phone_number, $smsMessage)) {
				...
				$_SESSION['showTokenInputField'] = true;
			} else {
...
<title><?php echo $_SESSION['user_full_name']." Profile"; ?></title>
...
<link href="../stylesheets/userEditPageStyle.css" rel="stylesheet" type="text/css" />
	            ...
	            <div class="CollapsiblePanelsDiv">
	              ...
	              <div id="CollapsiblePanel5" class="CollapsiblePanel">
	                ...
	                <div class="CollapsiblePanelContent" >
	                  <form action="" method="post" enctype="application/x-www-form-urlencoded" name="password_form" class="customer_label">
											<?php echo $security->csrf_token_tag(); ?>
	              ...
	              <div id="CollapsiblePanel6" class="CollapsiblePanel">
	                ...
	                <div class="CollapsiblePanelContent">
	                  <form action="" method="post" enctype="application/x-www-form-urlencoded" name="email_form" class="customer_label">
											<?php echo $security->csrf_token_tag(); ?>
	              ...
	              <div id="CollapsiblePanel7" class="CollapsiblePanel">
	                ...
	                <div class="CollapsiblePanelContent">
	                  <form action="" method="post" enctype="application/x-www-form-urlencoded" name="phone_number_form" class="customer_label">
					    				<?php echo $security->csrf_token_tag(); ?>
		                    ...
		                    <button name="clear_phoneNumber" id="clear_phoneNumber" class="clearBtn btnStyle1" >Clear</button>
											...
											<?php
												...
												if (isset($phone_validated) && !$phone_validated) { ?>
													...
													<input style="color:red; float:none" name="verifyNumber" type="submit" id="verifyNumber" class="btnStyle1" value="Verify phone number" /> <?php
												}
											?>
											
											<?php 
												if ($_SESSION['showTokenInputField']) { ?>
													<br/> <br/>
													<p> Enter the token received from your phone in the input field below. </p>
													<input style="float:none" name="smsToken" type="text" id="smsToken" size="60" maxlength="60" />
