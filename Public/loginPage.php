...
// require_once('../includes/google-login/vendor/autoload.php');
...
if (request_is_get() || request_is_post()) {
  ...
  if ($profileType === 'user') {
		if ($_SERVER["SERVER_NAME"] === 'localhost') {
			...
			defined('FB_REDIRECT_URI') ? null : define( 'FB_REDIRECT_URI', 'https://localhost:81/Public/loginPage.php?profile=user' );

			// Define the google redirect URI
			$google_client->setRedirectUri('http://localhost:81/Public/loginPage.php?profile=user');
		} else {
			// Define the facebook redirect URI for the user login portal
			defined('FB_REDIRECT_URI') ? null : define( 'FB_REDIRECT_URI', 'https://whosabiwork.com/Public/loginPage.php?profile=user' );

			// Define the google redirect URI
			$google_client->setRedirectUri('https://whosabiwork.com/Public/loginPage.php?profile=user');
		}					
		...
		if ($_SERVER["SERVER_NAME"] === 'localhost') {
			...
			// Define the google redirect URI
			$google_client->setRedirectUri('http://localhost:81/Public/loginPage.php?profile=customer');
		} else {
			...
			// Define the google redirect URI
			$google_client->setRedirectUri('https://whosabiwork.com/Public/loginPage.php?profile=customer');
		}
	}
	
	if (request_is_get()) {
		// Facebook $_GET request contains state, but google doesn't, so use it to differentiate the two requests
		// Both Facebook and Google have 'code' in the returned $_GET request
  	...
  	// Checking for 'authuser' in the get request distinguishes google's $_GET request from Facebook's $_GET request
  	if (isset($_GET["code"]) && isset($_GET["authuser"])) {
  		$googleLogin = $_SESSION['googleLogin'] = tryAndLoginWithGoogle($_GET);
  	}
  }
}
		...
		if (isset($post_params['userSignIn'])) { // Form has been submitted.
			...
			if (empty($validate->errors)) {
              ...
              clearFBsession(); 
						...
						updateGoogleUserAccessToken($found_user);
						...
						// check if the user is already adovacting a user before redirecting to the livePhotosFeed page
						$cusAdvocated = Advocate::find_all_advocated_by_user($found_user->id);
						// First check if it's from a redirect page
			...
			if (empty($validate->errors)) {
              ...
              clearFBsession(); 
						...
						updateGoogleUserAccessToken($found_customer);
...
function displayTabbedPanelsTab($profileType) {
	...
	if ($profileType == 'user') {
		$htmlOutput .= '<li class="TabbedPanelsTab" tabindex="0">User Sign In</li>';
	} else {
		$htmlOutput .= '<li class="TabbedPanelsTab" tabindex="0">Business Sign In</li>';
	}
...
function displayTabbedPanelsContent($profileType) {
	...
	if ($profileType === 'user') {
		...
		} elseif (isset( $_SESSION['request_to_connect_google'] ) && $_SESSION['request_to_connect_google']) {
			$htmlOutput .= '<div style="margin-bottom:10px; margin-left: 10px; margin-right: 10px;"> An account already exists with that email address. To connect your Google account, enter your password.
			</div>';
		}
		...
		$htmlOutput .= '<input name="username" type="text" id="username" size="60" maxlength="50" placeholder="Username / Email / Phone number" autocomplete="off" autocorrect="off" autocapitalize="none" value="';
			...
			} elseif (isset($_SESSION['google_user_info']['email']) && $_SESSION['request_to_connect_google']) {
				$htmlOutput .= $_SESSION['google_user_info']['email']; 
			} else {
		...
		$htmlOutput .= '</p>';
		$htmlOutput .= '<input name="userSignIn" type="submit" id="userSignIn" value="Sign In" />';
		
		$htmlOutput .= '<p id="loginInfoText" ><span>Or Sign In Using</span></p>';
        ...
        // Clear the session variables gotten from facebook
        clearFBsession();
	    } elseif (!empty( $_SESSION['googleLogin']['status'] ) && 'fail' == $_SESSION['googleLogin']['status']) {
	    	$htmlOutput .= $_SESSION['googleLogin']['message'];
        $htmlOutput .= ' <a style="text-decoration:none;" href="createUserAccount.php">Sign Up</a>';
        // Clear the session variables gotten from google
        clearGoogleSession();
	    }
		$htmlOutput .= '</div>';
	  ...
	  $htmlOutput .= '<a style="text-decoration: none;" href="'.$google_client->createAuthUrl().'">'; 
		...
		// Inform customer if check for facebook account found matches an email on the website
		if (isset( $_SESSION['eci_login_required_to_connect_facebook'] ) && $_SESSION['eci_login_required_to_connect_facebook']) {
			$htmlOutput .= '<div style="margin-bottom:10px; margin-left: 10px; margin-right: 10px;"> An account already exists with that email address. To connect your Facebook account, enter your password.
			</div>';
		} elseif (isset( $_SESSION['request_to_connect_google'] ) && $_SESSION['request_to_connect_google']) {
		...
		$htmlOutput .= '</p>';
		$htmlOutput .= '<input name="customerSignIn" type="submit" id="customerSignIn" value="Sign In" />';
